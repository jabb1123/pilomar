...
class DiskMonitor:  # 2 references.
    ...
    def __init__(
        ...
        self.log: LogFile = logger  # Which logger to use?
    ...
    def disc_ok(self, force=False) -> bool:
        ...
        megabytes = self.free_mega_bytes(force=force)
    ...
    def split_spaces(self, origline, sep=" "):
        ...
        for _, l in enumerate(origline):
            ...
            if (
                character == sep and not insidequotes
    ...
    def find_usb(self, devname="/dev/sda1"):
        ...
        if result:  # OK so far.
            ...
            if os.path.exists(
                ...
                if self.log is not None:
                    self.log(
                        "discmonitor.FindUSB:", self.df_path, "exists.", terminal=False
                    )
                ...
                c_cmd = "udisksctl mount -b " + devname  # Construct the mount command.
                print(
        ...
        if result:  # OK so far.
            dictionary = (
                self.get_df_dictionary()
            )  # Get the 'df' results from the operating system.
            if (
                self.df_path in dictionary
