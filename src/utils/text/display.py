...
class ColorDisplay:
    ...
    def __init__(
        ...
        first_scroll_row = When printing to window, this is the first row that will
                           scroll up as new lines are printed. (allows titles to stay fixed etc)
        title = Window title.
        ...
        -------------------------
        After instantiation, you can also set self.ClipWindow = True
           to allow the window to truncate display if insufficient realestate available.
        ...
        if isinstance(fg, list):
            ...
            self.deafult_foregrounds = [fg]
        self.foregroun_color_count = len(
            self.deafult_foregrounds
        )  # How many colors are available?
        self.foreground_color_index = (
            0  # Which color do we start with if multiple available?
        )
        if isinstance(bg, list):
            ...
            self.default_backgrounds = [bg]  # List of all background colors.
        self.background_color_count = len(
        ...
        # 1 means there's a title or something in row 0, etc. Scrolling takes this into account.
        ...
        # If TRUE the corners are highlighted in RED, and the FIELDS are highlighted in YELLOW(for layout checking)
        ...
        for c in range(self.display_columns):
            ...
            if self.window_title is not None:
                self.foreground_colors[0][
                    c
                ] = self.title_foreground  # Invert colors for titles.
                self.background_colors[0][
                    c
                ] = self.title_background  # Invert colors for titles.
            else:
                self.foreground_colors[0][
                    c
                ] = self.default_foreground  # Regular colors if no title.
                self.background_colors[0][
                    c
                ] = self.default_background  # Regular colors if no title.
        ...
        for f in self.fields:
            if f.name == name:  # Will update multiple fields with the same name.
                ...
                if f.type == "ProgressBar":
                    pval = float(
                        max(
                            min(self.get_float_value(value), f.progress_bar_max),
                            f.progress_bar_min,
                        )
                    )  # Limit value to progress bar limits.
                    if fg is None:
                        fg = (
                            f.progress_bar_foreground
                    ...
                    pc = (
                        round(
                            f.length
                            * (pval - f.progress_bar_min)
                            / (f.progress_bar_max - f.progress_bar_min)
                        )
                        - 1
    ...
    def copy_field_color(self, fromname, toname):
        ...
        if fromfield is not None and tofield is not None:  # Transfer the colors.
            result = self.field_color(
                toname, fg=f.foreground_color, bg=f.background_color
            )
        return result
    ...
    def scroll_up(self, lines=1, immediate=False):
        ...
        for i in range(lines):
            ...
            self.foreground_colors.append(
                [
                    self.deafult_foregrounds[self.foreground_color_index]
                    for c in range(self.display_columns)
                ]
            )
            self.background_colors.append(
                [
                    self.default_backgrounds[self.background_color_index]
                    for c in range(self.display_columns)
                ]
            )
        if immediate:
    ...
    def display(self, screenheight=None, screenwidth=None, immediate=False):
        ...
        if (
            ...
            if (
                not self.clip_window and maxscreenrow <= self.last_display_row
        ...
        for r in range(
            ...
            if self.draw_border and self.last_display_col + 1 < maxscreencol:
                line += TextColor.fgbgcolor(
                    self.border_foreground, self.border_background, vertical_char
                )
            if (
                not self.reduce_io or self.prev_line_strings[r] != line
        ...
        if self.draw_border and self.last_display_row + 1 < maxscreenrow:
            ...
            if (
                ...
                line = TextColor.fgbgcolor(
                    self.border_foreground,
                    self.border_background,
                    (horizontal_char * visiblecolumns),
                )  # Truncate the line.
