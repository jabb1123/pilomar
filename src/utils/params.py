...
class Parameters(AttributeMaster):  # Common
  ...
  def load_parameters(self):
    ...
    self.camera_enabled = self.get_parm_val("CameraEnabled", True)  # Is the camera on?
    ...
    self.ir_filter = self.get_parm_val("IRFilter", True)  # Is Infrared filter fitted?
  ...
  def set_camera_driver(self, cameradriver):
    ...
    if cameradriver in self.camera_commands:  # CameraDriver is recognised.
      ...
      self.camera_image_types = self.camera_commands[self.camera_driver]["imagetypes"]
  ...
  def show_color_scheme(self):
    ...
    print(TextColor.yellow("Current color scheme (" + str(self.color_scheme) + "):"))
    print(
      TextColor.fgbgcolor(self.menu_title_fg, self.menu_title_bg, " Menu title    "),
      self.menu_title_fg,
      "/",
      self.menu_title_bg,
    )
