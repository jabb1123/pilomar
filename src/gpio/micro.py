...
class Microcontroller(AttributeMaster):
    ...
    def __init__(self, port="/dev/serial0", resetpin=4, boardtype=None, logger=None):
        ...
        self.queue_from_mctl = (
            Queue()
        )  # Use queue mechanism to receive commands from the microcontroller communication thread.
        self.reset_bcm = resetpin  # Grounding this pin will RESET the remote device. (or turn it off if microcontroller power is controlled by it).
        self.reset_pin = outputpin(
            self.reset_bcm, "MctlReset"
        )  # Create GPIO pin if a pin is specified, else create dummy pin. All pins start OFF.
        self.lines = []  # No lines received yet.
        self.write_chunk_bytes = 32  # Maximum number of characters to send in a batch.
        self.write_chunk_seconds = (
            0.2  # Seconds between chunks written to microcontroller.
        )
        self.input_line = ""  # This is the line currently being received. Completed lines are added to Lines list.
        self.write_queue = []  # No output to send yet.
        self.lines_received = (
            0  # total number of lines received from the microcontroller.
        )
        self.lines_sent = 0  # Total count of lines sent to the microcontroller.
        self.bytes_received = 0  # Byte count from microcontroller.
        self.bytes_sent = 0  # Byte count sent to the microcontroller.
        self.print_comms = False  # When TRUE communication log is copied to the terminal, otherwise it's only written to the log file.
        self.led_status = True  # LEDS on by default.
        self.line_opened_time = now_utc()  # When did the UART comms start?
        self.last_tx_time = now_utc()  # When was data last sent?
        self.last_rx_time = now_utc()  # When was data last received?
        self.rx_errors = 0  # How many receive errors have been detected.
        self.last_line_sent = None  # We get 'reflection' on the UART port if the remote device isn't ready. This helps us to detect that.
        self.comms_timeout = (
            Parameters.MctlCommsTimeout
        )  # Seconds. microcontroller is restarted if no data received after this period.
        self.forced_restarts = 0  # How many restarts have been forced by this software?
        self.remote_restarts = (
            0  # How many restarts have been registered by the remote device itself?
        )
        self.reset_attempts = 0  # Increment for each sequential attempt to reset communication with the remote microcontroller board.
        self.powered_by_usb = False  # If the device is connected by USB, then power handling is different.
        ...
        for dn, dl in UsbList:  # Check all connected USB devices.
            if dl in [
                ...
                self.powered_by_usb = True  # Don't allow GPIO power pin to be used.
                print(
        ...
        if (
            ...
            self.reset_pin.On()  # Turn the microcontroller ON if we're in charge of the power supply.
        self.device_failure = (
        ...
        self.send_id = 0  # Incremental counter, the message number being sent to the microcontroller. The microcontroller will respond that this message number has been received.
    ...
    def StartMonitor(self):
        ...
        self.print_comms = True  # UART comms will be echoed to the terminal.
    ...
    def EndMonitor(self):
        ...
        self.print_comms = False  # UART comms will not be echoed to the terminal.
    ...
    def Initiate(self):
        ...
        self.set_clock()  # Set the time on the microcontroller.
    ...
    def Reset(self, planned=False):
        ...
        if (
            ...
            if self.reset_attempts > 10:  # Don't bother again.
                ...
                if self.error_window != None and hasattr(self.error_window, "Print"):
                    self.error_window.Print(msg)
    ...
    def ReadPoll(self):
        ...
        while self.uart.in_waiting:  # Something in the read queue.
            ...
            if response == "\n":  # End of line received.
                if (
                    self.input_line == self.last_line_sent
    ...
    def WriteFlush(self, send=True):
        ...
        if send:  # We should try to send outstanding messages.
            ...
            TryCount = 0
            self.write_prohibited = True  # Don't allow anything further to be added to the queue at the moment.
            while len(self.write_queue) > 0:
                ...
                self.write_poll()  # Send next chunk of data from output buffer if allowed.
                time.sleep(
                    0.1
        ...
        self.write_queue = []  # Delete any remaining messages in the queue.
        return result
    ...
    def write(self, line):
        ...
        if self.write_prohibited:
            ...
            line = line.replace("\n", "")  # Was strip()
            self.send_id += 1  # Increment the message ID number.
            if line[-1:] != " ":
    ...
    def CommsLoop(self, commandqueue):  # Runs as own thread.
        ...
        while True:  # Loop until explicitly told to break.
            ...
            self.write_poll()  # Send next chunk of data from output buffer if allowed.
            self.read_poll()  # Read anything waiting in the input buffer.
            if (
            ...
            if (
                ...
                received_message = commandqueue.get()
