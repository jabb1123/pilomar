...
class LocalStars(AttributeMaster):
    ...
    def __init__(
        ...
        self.log(f"LocalStars.__init__({ra}, {dec}, {radius}, {magnitude}, {maxstars}):", terminal=False)
    ...
    def update(self, ra: float, dec: float) -> bool:
        ...
        self.log(f"LocalStars.update: CoreSelection RA {self.min_ra_deg}° to {self.max_ra_deg}°, "
                f"Dec {self.min_dec_deg}° to {self.max_dec_deg}°", terminal=False)
        ...
        if self.min_ra_deg < 0:
            self.log(f"LocalStars.update: -ve RA Selection {self.min_ra_deg + 360}° to 360°", terminal=False)
            ...
            self.log(f"LocalStars.update: Appending {len(df1)} stars (<0 rule)", terminal=False)
            self._df = pandas.concat([self._df, df1])
            
        if self.max_ra_deg > 360:
            self.log(f"LocalStars.update: +ve RA Selection 0° to {self.max_ra_deg - 360}°", terminal=False)
            ...
            self.log(f"LocalStars.update: Appending {len(df2)} stars (>360 rule)", terminal=False)
    ...
    def column_index(self, name: str) -> Optional[int]:
        ...
        if name not in self.column_names:
            self.log(f"LocalStars.column_index: {name} not in {self.column_names}", 
    ...
    def get(self, ra: float, dec: float):
        ...
        if (abs(self.ra - ra) > self._update_angle or 
            abs(self.dec - dec) > self._update_angle):
    ...
    @MasterDf.setter
    def MasterDf(self, value):
        self.master_df = value
        
    @property
    def StarFilter(self):
        return self.star_filter
    
    @StarFilter.setter
    def StarFilter(self, value):
