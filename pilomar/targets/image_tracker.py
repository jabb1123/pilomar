...
class ImageTracker(AttributeMaster):
    ...
    def __init__(
        ...
        self._timestamp_func = timestamp_func or (lambda: datetime.utcnow().strftime('%Y%m%d%H%M%S'))
    ...
    def reset(self):
        ...
        if self._image_class:
            self.target_image = self._image_class(name='target', logger=self._logger)
        else:
        ...
        if self._image_class:
            self.master_image = self._image_class(name='master', logger=self._logger)
        else:
        ...
        # Reset Latest Image
        if self._image_class:
            self.latest_image = self._image_class(name='latest', logger=self._logger)
        else:
    ...
    def set_master_image(
        ...
        self.log(f"ImageTracker.set_master_image: Received buffer type {type(cv_image_buffer)}", terminal=False)
        
        if cv_image_buffer is None:
            self.log("ImageTracker.set_master_image: Received None buffer. Nothing set.", terminal=False)
            return
        ...
        if self.master_image:
            ...
            self.log(f"ImageTracker.set_master_image: Loaded image {self.master_image.get_dimensions()}", terminal=False)
            self.master_image.change_type('grayscale')
            
            # Measure contrast
            contrast_m, contrast_s = self.master_image.measure_contrast()
            self.log(f"ImageTracker.set_master_image: Contrast {contrast_m}, {contrast_s}", terminal=False)
        ...
        if min_magnitude is not None:
            self.log(f"ImageTracker.set_master_image: Setting min_magnitude to {min_magnitude}", terminal=False)
        ...
        if self.master_image:
            if starcount is None or starlist is None:
                self.log("ImageTracker.set_master_image: Calculating stars from image", terminal=False)
                self.master_image.count_stars()
            else:
                self.master_image.star_count = starcount
                self.master_image.star_list = starlist
                
            self.log(f"ImageTracker.set_master_image: Counted {self.master_image.star_count} stars", terminal=False)
            
            # Save reference image
            if self._folder_handler:
                filename = self._folder_handler.prep_file(
                    'tracking',
                    f'MasterTrackingImage_{self._timestamp_func()}.jpg'
                )
                if self._camera_window:
                    now_hms = datetime.utcnow().strftime('%H:%M:%S')
                    self._camera_window.print(f"{now_hms} {filename.split('/')[-1]}")
        ...
        self.log(f"ImageTracker.set_master_image: search_master_image returned {result}", terminal=False)
    ...
    def set_target_image(self, cv_image_buffer, timestamp=None, zone: int = 0):
        ...
        if self.target_image:
            ...
            self.log(f"ImageTracker.set_target_image: Loaded {self.target_image.get_dimensions()}", terminal=False)
            self.target_image.change_type('grayscale')
            
            # Count stars
            self.target_image.count_stars()
            self.log(f"ImageTracker.set_target_image: Counted {self.target_image.star_count} stars", terminal=False)
    ...
    def set_latest_image(self, cv_image_buffer, timestamp=None):
        ...
        if self.latest_image:
            ...
            self.log(f"ImageTracker.set_latest_image: Loaded {self.latest_image.get_dimensions()}", terminal=False)
            
            # Measure contrast
            contrast_m, contrast_s = self.latest_image.measure_contrast()
            self.log(f"ImageTracker.set_latest_image: Contrast {contrast_m}, {contrast_s}", terminal=False)
            ...
            if self._parameters:
                ...
                if filter_name:
                    if self.latest_image.run_filter_script(scriptname=filter_name, window=self._dev_window):
                        self.log(f"ImageTracker.set_latest_image: Filter {filter_name} success", terminal=False)
                    else:
                        self.log(f"ImageTracker.set_latest_image: Filter {filter_name} failed", level='warning')
            ...
            self.log(f"ImageTracker.set_latest_image: Counted {self.latest_image.star_count} stars", terminal=False)
            
            # Save for reference
            if self._folder_handler:
                filename = self._folder_handler.prep_file('tracking', f'LatestTrackingImage_{uts}.jpg')
                if self._camera_window:
                    now_hms = datetime.utcnow().strftime('%H:%M:%S')
                    self._camera_window.print(f"{now_hms} {filename.split('/')[-1]}")
    ...
    def choose_search_sequence(self):
        ...
        self.log(f"ImageTracker.choose_search_sequence: Master {master_width}x{master_height}, "
                f"Latest {latest_width}x{latest_height}", terminal=False)
        ...
        while col_count <= 10:  # Safety limit
            ...
            while row_count <= 10:  # Safety limit
                import math
                dist = math.sqrt((zone_center_x - master_center_x) ** 2 + 
                ...
                if row_count > 0 and col_count > 0:
                    mirror_list.append([master_width - zone_center_x, master_height - zone_center_y])
        ...
        self.log(f"ImageTracker.choose_search_sequence: {len(self.zone_list)} zones", terminal=False)
    ...
    def create_target_zone_buffer(self, zone: int):
        ...
        if zone < 0 or zone >= len(self.zone_list):
            self.log(f"ImageTracker.create_target_zone_buffer: Invalid zone {zone}", 
        ...
        if self.master_image and self.master_image.image_buffer is not None:
            ...
            self.log(f"ImageTracker.create_target_zone_buffer: Created {image_buffer.shape}", terminal=False)
            return image_buffer
    ...
    def search_master_image(self) -> bool:
        ...
        """
        self.log("ImageTracker.search_master_image: Begin", terminal=False)
        
        if not ASTROALIGN_AVAILABLE:
            self.log("ImageTracker.search_master_image: astroalign not available", level='error')
            return False
            
        self.choose_search_sequence()
        num_zones = len(self.zone_list)
        self.log(f"ImageTracker.search_master_image: {num_zones} zones available", terminal=False)
        ...
        for i, entry in enumerate(self.zone_list):
            ...
            try:
                if (self.target_image and self.latest_image and 
                    self.target_image.image_buffer is not None and 
                    self.latest_image.image_buffer is not None):
                    ...
                    self.log(f"ImageTracker.search_master_image: Zone {i}: "
                            f"{len(tsl)} target stars, {len(lsl)} latest stars", terminal=False)
                    ...
                    self.log(f"ImageTracker.search_master_image: Zone {i}: "
                            f"dx={dx}, dy={dy}, true_dx={true_dx}, true_dy={true_dy}", terminal=False)
                    
                    self.save_tracking_analysis(zone=i)
                    result = True
                    
            except Exception as e:
                self.log(f"ImageTracker.search_master_image: Zone {i}: Error: {e}", terminal=False)
                
            if good_calculations >= target_matches:
                self.log(f"ImageTracker.search_master_image: {good_calculations} matches, stopping", terminal=False)
                break
        ...
        self.log(f"ImageTracker.search_master_image: Average drift x={self.dx}, y={self.dy} "
                f"across {good_calculations} alignments", terminal=False)
    ...
    def SetMasterImage(self, *args, **kwargs):
        return self.set_master_image(*args, **kwargs)
    
    def SetTargetImage(self, *args, **kwargs):
        return self.set_target_image(*args, **kwargs)
    
    def SetLatestImage(self, *args, **kwargs):
        return self.set_latest_image(*args, **kwargs)
    
    def ChooseSearchSequence(self):
        return self.choose_search_sequence()
    
    def CreateTargetZoneBuffer(self, zone):
        return self.create_target_zone_buffer(zone)
    
    def SearchMasterImage(self):
        return self.search_master_image()
    
    def SaveTrackingAnalysis(self, zone=0):
        return self.save_tracking_analysis(zone)
    
    def ValidStarValues(self, entry):
        return self.valid_star_values(entry)
    
    # Legacy attribute aliases
    @property
    def TrackingInterval(self):
        return self.tracking_interval
    
    @property
    def TargetMinMagnitude(self):
        return self.target_min_magnitude
    
    @TargetMinMagnitude.setter
    def TargetMinMagnitude(self, value):
        self.target_min_magnitude = value
