...
class PilomarImage:
    ...
    def orient_height(self,y,height=None):
        ...
        if self.invert_height: # Co-ordinates provided are from BOTTOM UP, convert to TOP DOWN for OpenCV.
            if height == None: height = self.get_height() - 1
    ...
    def orient_coord(self,location,yloc=1,angle=0,height=None):
        ...
        if yloc == 1: # y location is at position 1 in the tuple, convert that.
            x = location[0]
            y = self.orient_height(location[1]) # Does height increase from the TOP or BOTTOM of the image?
            r = (x,y)
        else: # y location is at position 0 in the tuple, convert that.
            x = location[1]
            y = self.orient_height(location[0]) # Does height increase from the TOP or BOTTOM of the image?
            r = (y,x)
        return r
    ...
    def reset_graph(self):
        ...
        self.graph_data_sets = [] # DataPoint = [x,y,color,label,style] # DataSet = list of DataPoint # DataSets = list of DataSet
    ...
    def map_to_graph(self,xpoint,ypoint):
        ...
        x = int(self.interpolate(self.graph_x_min_val,self.graph_x_border,self.graph_x_max_val,self.get_width() - self.graph_x_border,xpoint)) # Scale it to the size of the canvas.
        y = int(self.interpolate(self.graph_y_min_val,self.graph_y_border,self.graph_y_max_val,self.get_height() - self.graph_y_border,ypoint)) # Scale it to the size of the canvas.
        return x,y
        
    def draw_x_axis(self):
        """ Draw X axis, scale and label """
        # Draw X scale where Y = 0 or at min Y
        if self.graph_y_min_val < 0 and self.graph_y_max_val > 0: y = 0 # Where does x axis cross Y?
        elif self.graph_y_max_val < 0: y = self.graph_y_max_val
        else: y = self.graph_y_min_val
        ...
        while i <= self.graph_x_max_val:
            ...
            self.add_text(str(round(i,3)),x1,y1 - 30,color=PilomarImage.BGRColor['Black'],hjust='c',vjust='t')
            i += self.graph_x_ticks
        # Label the axis.
        cx, _ = self.CenterCoordinates()
        self.add_text(self.graph_x_axis_title,cx,int(self.graph_y_border / 2),color=PilomarImage.BGRColor['Black'],size=2.0,hjust='c',vjust='c',thickness=2)
        return True

    def draw_y_axis(self):
        """ Draw X axis, scale and label """
        # Draw Y scale where X = 0 or at min X
        if self.graph_x_min_val < 0 and self.graph_x_max_val > 0: x = 0 # Where does Y axis cross X?
        elif self.graph_x_max_val < 0: x = self.graph_x_max_val
        else: x = self.graph_x_min_val
        ...
        while i <= self.graph_y_max_val:
            ...
            self.add_text(str(round(i,3)),x1 - 30,y1,color=PilomarImage.BGRColor['Black'],hjust='r',vjust='c')
            i += self.graph_y_ticks
        # Label the axis.
        _, cy = self.CenterCoordinates()
        self.add_angle_text(self.graph_y_axis_title,int(self.graph_x_border / 2),cy,color=PilomarImage.BGRColor['Black'],size=2.0,hjust='c',vjust='c',thickness=2,angle=90) # Rotated. Not working smoothly yet.
        return True
    ...
    def ListDataSets(self):
        ...
        self.add_text("Datasets:-",x,self.get_height() - 500,color=PilomarImage.BGRColor['Black'])
        for dataset in self.graph_data_sets:
            self.add_text(dataset.name,x,self.PrevTextY,color=dataset.Color)
        return True
    ...
    def AnalyseDataPoints(self):
        ...
        self.graph_x_min_val = self.graph_x_max_val = self.graph_y_min_val = self.graph_y_max_val = None
        if len(self.graph_data_sets) > 0: # There are datasets to handle.
            for dataset in self.graph_data_sets: # Data point = [x,y,color,label,style] # DataSet = list of DataPoints, # DataSets = list of DataSets
                if len(dataset.DataPoints) > 0: # There are datapoints to handle.
                    for point in dataset.DataPoints: # Check each point.
                        if self.graph_x_min_val == None or self.graph_x_min_val > point.X: self.graph_x_min_val = point.X
                        if self.graph_y_min_val == None or self.graph_y_min_val > point.Y: self.graph_y_min_val = point.Y
                        if self.graph_x_max_val == None or self.graph_x_max_val < point.X: self.graph_x_max_val = point.X
                        if self.graph_y_max_val == None or self.graph_y_max_val < point.Y: self.graph_y_max_val = point.Y
        if self.graph_x_min_val == None: # No values were found.
        ...
        for dataset in self.graph_data_sets:
            ...
            for i,datapoint in enumerate(dataset.DataPoints):
                ...
                if 'line' in dataset.Style and i > 0: # Line between points.
                    self.draw_line((prevx,prevy),(x,y),color=self.SafeColor(dataset.Color,default=PilomarImage.BGRColor['Cyan']))
    ...
    def plot_graph(self,height,width,filename,export=False):
        ...
        self.invert_height = True # Easier to plot graphs if HEIGHT pixels count from the bottom up.
        # Find axis limits.
        ...
        self.draw_rectangle((self.graph_x_border,self.graph_y_border),(width - self.graph_x_border,height - self.graph_y_border),color=PilomarImage.BGRColor['DarkGray'])
        self.draw_x_axis() # Draw X axis on graph.
        self.draw_y_axis() # Draw Y axis on graph.
        x,y = self.CenterCoordinates()
        self.add_text(self.graph_title,x,int(height - self.graph_y_border / 2),size=3,color=PilomarImage.BGRColor['Black'],thickness=3,hjust='c',vjust='c')
    ...
    def DrawGraphID(self):
        ...
        self.add_text(line,self.get_width() - 10, 20, color=PilomarImage.BGRColor['Black'],hjust='r')
        return True
    ...
    def _initialize(self):
        ...
        self.action_list = [['__version__',str(PilomarImage.__version__)]] # List of actions performed on the image.
        self.created_timestamp = self.NowUTC()
    ...
    def CalculateStarSpread(self):
        ...
        if self.image_exists() and len(self.StarList) > 0: # There's an image loaded and stars were identified.
            ...
            self.HorizontalSpread = 100 * (HMax - HMin) / self.get_width()
            self.VerticalSpread = 100 * (VMax - VMin) / self.get_height()
    ...
    def NextInterpolation(self):
        ...
        self.action_list.append(['nextinterpolation',self.ResizeMethod])
    ...
    def PrevInterpolation(self):
        ...
        i = (self.ResizeMethods.index(self.ResizeMethod) - 1) % len(self.ResizeMethods)
        self.ResizeMethod = self.ResizeMethods[i]
        self.action_list.append(['previnterpolation',self.ResizeMethod])
    ...
    def Clear(self):
        ...
        self.created_timestamp = self.NowUTC()
        self.modified_timestamp = self.NowUTC()
    ...
    def load_buffer(self,imagebuffer):
        ...
        if type(imagebuffer) != type(None):
            ...
            self.log("pilomarimage",self.name,".load_buffer(). FROM buffer is None.",terminal=False)
    ...
    def AccumulateBuffer(self,buffer):
        ...
        if isinstance(self.ImageAccumulator,type(None)): # Initialize accumulator.
            self.ImageAccumulator = np.zeros_like(buffer.image_buffer,np.uint16) # Create array of same dimensions, but with larger storage type.
            self.ImageCounter = np.zeros_like(buffer.image_buffer,np.uint8) # Create array of same dimensions but with uint8 storage type.
        # Now accumulate the values.
        self.ImageAccumulator.add(self.ImageAccumulator,buffer.image_buffer)
        self.ImageCount += 1
    ...
    def ResolveAccumulator(self):
        ...
        self.image_buffer = np.zeros_like(self.ImageAccumulator,np.uint8) # Create array of same dimensions, but with regular image datatype.
        self.image_buffer[self.ImageCount != 0] = self.ImageAccumulator / self.ImageCount
    ...
    def get_exif(self,filename):
        ...
        if piexif != None: # piexif is available.
            ...
            self.log("PilomarImage.get_exif(",filename,") piexif not available.",terminal=False)
    ...
    def load_file(self,filename,loadexif=False):
        ...
        if self.image_exists():
            self.ImageMask = np.ones_like(self.image_buffer,np.uint8) # All cells are active.
            ...
            self.log("pilomarimage",self.name,".load_file(): Loaded",self.GetDimensions(),self.GetType(),terminal=False)
        else:
            # File didn't load!
            self.log("pilomarimage",self.name,".load_file(",filename,") failed.",terminal=False)
            result = False
        return result
    ...
    def save_file(self,filename,quality=None,library='opencv',exif_dict=None):
        ...
        if self.image_exists():
            if library == 'pil': # Use Pillow to write the image file.
                self.log("pilomarimage",self.name,".save_file(",filename,") via PIL.",terminal=False)
                pil_buffer = self.OpenCVtoPIL(self.image_buffer) # Get buffer as a PIL object.
                ...
                self.log("pilomarimage",self.name,".save_file(",filename,") via OpenCV.",terminal=False)
                if quality != None: # Image quality was specified.
                    cv2.imwrite(filename,self.image_buffer,[int(cv2.IMWRITE_JPEG_QUALITY), int(quality)]) # imwrite doesn't report errors very well, beware.
                else: # Image quality can be default.
                    cv2.imwrite(filename,self.image_buffer) # imwrite doesn't report errors very well, beware.
            self.action_list.append(['save',filename])
        else:
            print("PilomarImage.save_file(",filename,"): No ImageBuffer.")
        ...
        if ift in ['bmp'] and maxdim > 32768: # bmp dimensions can't exceed this size.
            print("PilomarImage.save_file(",filename,"): Image dimensions exceed bmp limits.(",height,width,")")
        elif ift in ['jpg','jpeg'] and maxdim > 65535: # jpeg dimensions can't exceed this size.
            print("PilomarImage.save_file(",filename,"): Image dimensions exceed jpeg limits.(",height,width,")")
        # Check it worked. Big images fail silently!
        result = False # Failed unless a file exists which contains something.
        if os.path.exists(filename):
            if os.path.getsize(filename) == 0:
                print("PilomarImage.save_file(",filename,"): The file exists but it is empty.")
            else:
                result = True
        else:
    ...
    def clip_image(self,xstart,ystart,xend,yend):
        """ Clip the image. """
        self.log("pilomarimage",self.name,".clip_image(",xstart,ystart,xend,yend,")",terminal=False)
        gt = self.GetType()
        if gt == 'grayscale': self.image_buffer = self.image_buffer[ystart:yend,xstart:xend]
        else: self.image_buffer = self.image_buffer[ystart:yend,xstart:xend,:]
    ...
    def scale_image(self,scale=None,vscale=None,hscale=None,width=None,height=None):
        ...
        self.image_buffer = cv2.resize(self.image_buffer,(width,height),interpolation=self.ResizeMethod) # Note RESIZE takes (width,height) rather than usual openCV (height,width)!
    ...
    def HorizontalBlurImage(self,band):
        ...
        self.image_buffer = cv2.resize(self.image_buffer,(width,height),interpolation=cv2.INTER_AREA) # INTER_AREA better for SHRINKING.
        self.image_buffer = cv2.resize(self.image_buffer,(originalwidth,height),interpolation=cv2.INTER_LINEAR) # INTER_LINEAR and INTER_CUBIC best for STRETCHING.
    ...
    def CloneImage(self,donor):
        ...
        if isinstance(donor.image_buffer,type(None)): self.image_buffer = None 
        else: self.image_buffer = donor.image_buffer.copy()
    ...
    def MergeLayer(self,donor):
        ...
        if gt == 'grayscale': # Grayscale images inherit the average of the two arrays.
            ...
            b1 = self.image_buffer.copy().astype(np.float16) / 255 # Scale everything 0.0 - 1.0
            ...
            self.image_buffer = self.image_buffer.astype(np.uint8) # Convert from float back to uint8
    ...
    def LineDetection(self,markupfile=None):
        ...
        elif self.image_buffer.shape[2] == 3: markup_color = (255,255,255) # 'bgr'
        ...
        blur = cv2.gaussian_blur(gray, (5,5), 0)
    ...
    def CloudDetection(self,threshold=127): # In pilomarimage
        ...
        # - Blur
        imagebuffer = cv2.gaussian_blur(imagebuffer,(5,5),0)
    ...
    def ImageAge(self):
        ...
        td = None
        if self.created_timestamp != None: td = int((self.NowUTC() - self.created_timestamp).total_seconds())
        return td
    ...
    def ChangeType(self,newtype):
        ...
        self.image_buffer = self.NewBufferType(newtype)
        self.ImageMask = np.ones_like(self.image_buffer,np.uint8)
        self.action_list.append(['changetype',self.GetType()])
        return True
    ...
    def get_depth(self):
        ...
        if len(self.image_buffer.shape) < 3: # Grayscale
            depth = 1
        else: depth = self.image_buffer.shape[2] # BGR = 3 or BGRA = 4
        return depth
    ...
    def GetPixelColor(self,y,x,trusted=True):
        ...
        try:
            if tg == 'grayscale': color = (int(self.image_buffer[y,x]),int(self.image_buffer[y,x]),int(self.image_buffer[y,x]))
            else: color = (int(self.image_buffer[y,x,0]),int(self.image_buffer[y,x,1]),int(self.image_buffer[y,x,2]))
        except Exception as e:
    ...
    def New(self,height,width,imagetype='bgr',datatype=np.uint8):
        ...
        if imagetype == 'bgr': self.image_buffer = np.zeros((height,width,3), datatype) # bgr image.
        elif imagetype == 'bgra': self.image_buffer = np.zeros((height,width,4), datatype) # bgra image.
        else: self.image_buffer = np.zeros((height,width), datatype) # grayscale image.
        self.ImageMask = np.ones_like(self.image_buffer,np.uint8)
        self.created_timestamp = self.NowUTC()
        self.modified_timestamp = self.NowUTC()
        self.exif_data = {} # Empty dictionary of any associated EXIF tags loaded from an image.
    ...
    def count_stars(self,minval=3,maxval=650,maxstars=500,threshold=100):
        ...
        self.log("pilomarimage",self.name,".count_stars(",minval,',',maxval,")",terminal=False)
    ...
    def mark_location(self,starx,stary,color,uppertext=None,lowertext=None):
        ...
        starx = int(starx)
        stary = int(stary)
        if starx < (self.get_width() / 2): xloc = starx + 10
        else: xloc = starx - 120
        yloc = stary
    ...
    def ImageExists(self):
        ...
        if isinstance(self.image_buffer,type(None)): return False
        else: return True

    def ImageMissing(self):
        """ Return TRUE if ImageBuffer is not initialized. """
        result = False
        if isinstance(self.image_buffer,type(None)): result = True
        return result
    ...
    def EnhanceStars(self,blurradius=13,cloudthresh=100,starthresh=16,maxval=255):
        ...
        retval, self.image_buffer = cv2.threshold(self.image_buffer,cloudthresh,maxval,cv2.THRESH_BINARY) # 100 should ignore clouds more easily and just recognise brighter stars.
        ...
        self.image_buffer = cv2.gaussian_blur(self.image_buffer,(blurradius,blurradius),0)
        ...
        retval, self.image_buffer = cv2.threshold(self.image_buffer,starthresh,maxval,cv2.THRESH_BINARY + cv2.THRESH_OTSU) # OTSU is adaptive threshold limits.
    ...
    def FS_Save(self,filterdata):
        ...
        if 'saveas' in filterdata: # There is a filename to use.
            ...
            if filename != None and len(filename) > 0:
                ...
                self.add_text("filterdata:",x=10,y=int(self.get_height() / 2),color=PilomarImage.BGRColor['White'],bgcolor=PilomarImage.BGRColor['Black'])
                for key,value in filterdata.items():
                    self.add_text(" " + str(key) + ":" + str(value),x=10,y=self.NextTextY,color=PilomarImage.BGRColor['White'],bgcolor=PilomarImage.BGRColor['Black'])
                self.Save(filename)
        return True
    ...
    def FS_Threshold(self,filterdata):
        ...
        calculatedthreshold, self.image_buffer = cv2.threshold(self.image_buffer,threshold,maxval,threshold_type)
    ...
    def FS_GaussianBlur(self,filterdata):
        ...
        self.image_buffer = cv2.gaussian_blur(self.image_buffer,(radius,radius),0)    
    ...
    def FS_Dehaze(self,filterdata):
        ...
        buffer = self.image_buffer.copy() # Copy the image buffer, we will blur this copy.
    ...
    def run_filter_script(self,scriptname,window=None):
        ...
        for entryname,filterdata in filterscript.items(): # Go through each set of filters in turn.
            self.log("PilomarImage.run_filter_script(",filtercount,entryname,") Running filter...",terminal=False) # Report the name of the filter
            # Each 'item' should be a sub-dictionary of a filter and its parameters to apply to the current image.
            ...
            if filtermethod == 'dehaze': result = self.FS_Dehaze(filterdata) # Remove haze from the image.
                ...
                self.log("PilomarImage.run_filter_script(",filtercount,entryname,") filtermethod",filtermethod,"does not exist.",level='error')
                print("**ERROR** PilomarImage.run_filter_script(",filtercount,entryname,") filtermethod",filtermethod,"does not exist.")
                result = False
            if not result: break # Failure.
            filtercount += 1 # Increment count.
        if not result:
            self.log("PilomarImage.run_filter_script(",scriptname,") did not complete successfully.",level='warning')
            print("WARNING: PilomarImage.run_filter_script(",scriptname,") did not complete successfully.")
            if hasattr(window,'Print'): window.Print("Filter script failed.") # Can report progress to a display window.
        return result
    ...
    def FakeField(self): # Generate fake field noise.
        ...
        self.image_buffer = np.clip(fieldimg,0,255).astype(np.uint8) # Clip to uint8 values.
    ...
    def FakeNoise(self): # Generate fake image noise.
        ...
        fieldimg = np.random.randint(0,25,(self.get_height(),self.get_width(),3),np.uint16) # 'bgr' buffer of random values.
    ...
    def FakeMeteor(self): # Generate fake meteor streak
        ...
        self.image_buffer = np.clip(meteorimg,0,255).astype(np.uint8) # Clip to uint8 values.
    ...
    def PlotStars(self,radius,starlist):
        ...
        for star_x, star_y, star_r in starlist:
            self.image_buffer = cv2.circle(self.image_buffer,(star_x,star_y),radius,GrayscaleWhite,thickness=-1) # White. All stars converted to standard 7 pixel radius.
    ...
    def draw_line(self,startcoord,endcoord,color=None,thickness=None,arrowpixels=None):
        ...
        if arrowpixels == None or distance <= 0:
            self.image_buffer = cv2.line(self.image_buffer, startcoord, endcoord, color, thickness=thickness, lineType=cv2.LINE_AA)
        else:
            ...
            self.image_buffer = cv2.arrowedLine(self.image_buffer, startcoord, endcoord, color, thickness=thickness, line_type=cv2.LINE_AA, tipLength=arrowproportion)
    ...
    def DrawEdgeLine(self,startcoord,endcoord,color=None,edgecolor=None,thickness=None,edgethickness=1,arrowpixels=None):
        ...
        endcoord = self.orient_coord(endcoord) # Make sure HEIGHT is right way up.
        if arrowpixels == None:
            self.image_buffer = cv2.line(self.image_buffer, startcoord, endcoord, edgecolor, thickness=int(thickness + (2 * edgethickness)), lineType=cv2.LINE_AA)
            self.image_buffer = cv2.line(self.image_buffer, startcoord, endcoord, color, thickness=thickness, lineType=cv2.LINE_AA)
        else:
            distance = math.sqrt(((endcoord[0] - startcoord[0]) ** 2) + ((endcoord[1] - startcoord[1]) ** 2))
            arrowproportion = arrowpixels / distance # ArrowedLine specifies arrow size as proportion of line length. We need constant 10pixel arrow heads.
            self.image_buffer = cv2.arrowedLine(self.image_buffer, startcoord, endcoord, edgecolor, thickness=int(thickness + (2 * edgethickness)), line_type=cv2.LINE_AA, tipLength=arrowproportion)
            self.image_buffer = cv2.arrowedLine(self.image_buffer, startcoord, endcoord, color, thickness=thickness, line_type=cv2.LINE_AA, tipLength=arrowproportion)
        self.action_list.append(['drawedgeline',startcoord,endcoord,color,edgecolor,thickness,edgethickness,arrowpixels])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def draw_circle(self,center_x,center_y,rad,color=None,thickness=None):
        ...
        self.image_buffer = cv2.circle(self.image_buffer,(center_x, center_y), rad, color, thickness=thickness, lineType=cv2.LINE_AA)
    ...
    def DrawEdgeCircle(self,center_x,center_y,rad,color=None,thickness=None,edgecolor=None,edgethickness=1):
        ...
        color = self.SafeColor(color)
        center_y = self.orient_height(center_y) # Make sure HEIGHT is right way up.
        self.image_buffer = cv2.circle(self.image_buffer,(center_x, center_y), rad, edgecolor, thickness=thickness + (2 * edgethickness), lineType=cv2.LINE_AA)
        self.image_buffer = cv2.circle(self.image_buffer,(center_x, center_y), rad, color, thickness=thickness, lineType=cv2.LINE_AA)
    ...
    def FillCircle(self,center_x,center_y,rad,color=None):
        ...
        color = self.SafeColor(color)
        center_y = self.orient_height(center_y) # Make sure HEIGHT is right way up.
        self.image_buffer = cv2.circle(self.image_buffer,(center_x, center_y), rad, color, thickness=-1, lineType=cv2.LINE_AA)
    ...
    def set_pixel(self,center_x,center_y,color=None,trusted=False):
        ...
        if not trusted: 
            if center_x < 0 or center_y < 0 or center_x >= self.get_width() or center_y >= self.get_height(): return True # Off the image, ignore.
        color = self.SafeColor(color)
        center_y = self.orient_height(center_y) # Make sure HEIGHT is right way up.
    ...
    def FadeCircle(self,center_x,center_y,rad,color=None,fadecolor=None):
        ...
        for i in range(rad, 0, -2):
            ...
            if gradedcolor != prevcolor:
                self.image_buffer = cv2.circle(self.image_buffer,(center_x, center_y), i, gradedcolor, thickness=-1, lineType=cv2.LINE_AA)
    ...
    def add_text_block(self,textlines,fromx,fromy,color=None,size=1.0,thickness=None,hjust='l',vjust='t',border=None,bgcolor=None):
        ...
        for i,line in enumerate(textlines):
            if i == 0: # 1st line. # Take over painting of border and background to make it a block instead.
                self.add_text(line,fromx,fromy,color=color,size=size,thickness=thickness,hjust=hjust,vjust=vjust,border=None,bgcolor=None)
            else: # subsequent lines.
                self.add_text(line,fromx,self.NextTextY,color=color,size=size,thickness=thickness,hjust=hjust,vjust=vjust,border=None,bgcolor=None)
        return True

    def add_text(self,text,fromx,fromy,color=None,size=1.0,thickness=None,hjust='l',vjust='t',border=None,bgcolor=None):
        """ Add text to an image.
            ...
            y co-ordinate of the next/prev line of text to generate. This takes font size into account.
                ...
                self.add_text('line2',x,self.NextTextY) # NextTextY contains the starting Y coordinate for the next line.
            ...
            If you are changing font sizes, it is best to work bottom-up using self.PrevTextY, the spacing works more dynamically this way.
                ...
                self.add_text('prevline',x,self.PrevTextY,size=2) # Print previous (higher) line next.
        ...
        if OK2Draw: # Proceed with the drawing.
            ...
            if border != None: #Draw a border around the text.
                self.draw_rectangle((x - border,y + ybase + border),(x + xdim + border,y - ydim + ybase - border),color=color,thickness=thickness)
            # Store where the 'next' line of text would go if we're printing multiple lines. (Text must remain same size!)
            self.image_buffer = cv2.putText(self.image_buffer,text,self.orient_coord((x,y)),self.font,size,color,thickness,lineType=cv2.LINE_AA)
            self.action_list.append(['addtext',text,fromx,fromy,color,size,thickness,hjust,vjust])
            self.modified_timestamp = self.NowUTC()
    ...
    def add_angle_text(self,text,fromx,fromy,color=None,size=1.0,thickness=None,hjust='l',vjust='t',border=None,bgcolor=None,angle=90):
        ...
        if not angle in supportedangles:
            print("PilomarImage.add_angle_text() only supports these angles",str(supportedangles))
            return False
        if self.ImageMissing(): print('pilomarimage',self.name,'.AddAngleText: No image in the buffer.')
        fromx = int(fromx)
        fromy = int(self.orient_height(fromy)) # Handle vertical orientation of pixel address here. Then it's simpler for all the following maths.
        if angle == 90:
            ...
            fromx,fromy = self.rotate_coordinates(fromx,fromy,360 - angle) # We counterrotated the image so that the text is written at the desired angle, so counterrotate the text coordinates too.
        thickness = self.SafeThickness(thickness)
        color = self.SafeColor(color)
        xdim,ydim,ybase = self.GetTextArea(text,size=size,thickness=thickness) # Boundaries of the text.
        ...
        if border != None: #Draw a border around the text.
            self.draw_rectangle((x - border,y + ybase + border),(x + xdim + border,y - ydim + ybase + border),color=color,thickness=thickness)
        self.image_buffer = cv2.putText(self.image_buffer,text,(x,y),self.font,size,color,thickness,lineType=cv2.LINE_AA) # *Q* <- Not implemented for angled text yet.
        # Store where the 'next' line of text would go if we're printing multiple lines. (Text must remain same size!)
        if angle == 90:
            # Now turn image the right way round again.
            self.rotate_image(angle)
        self.action_list.append(['addangletext',text,fromx,fromy,color,size,thickness,hjust,vjust,angle])
        self.modified_timestamp = self.NowUTC()
        return True

    def add_edge_text(self,text,fromx,fromy,color=None,edgecolor=None,size=1.0,thickness=None,edgethickness=None,hjust='l',vjust='t',border=None,bgcolor=None):
        ...
        if bgcolor != None: # Draw background under the text.
            ...
            self.FillRectangle((x - b,y + ybase + b),(x + xdim + b,y - ydim + ybase + b),color=bgcolor)
        if border != None: #Draw a border around the text.
            self.draw_rectangle((x - border,y + ybase + border),(x + xdim + border,y - ydim + ybase + border),color=edgecolor,thickness=edgethickness)
            self.draw_rectangle((x - border,y + ybase + border),(x + xdim + border,y - ydim + ybase + border),color=color,thickness=thickness)
        self.image_buffer = cv2.putText(self.image_buffer,text,self.orient_coord((x,y)),self.font,size,edgecolor,int(thickness + (2 * edgethickness)),lineType=cv2.LINE_AA)
        self.image_buffer = cv2.putText(self.image_buffer,text,self.orient_coord((x,y)),self.font,size,color,thickness,lineType=cv2.LINE_AA)
        # Store where the 'next' line of text would go if we're printing multiple lines. (Text must remain same size!)
        self.NextTextX = fromx # If printing multiple lines of text, this is the start point for the next line if you're printing downwards.
    ...
    def DrawEdgePolygon(self,pointlist,color=None,edgecolor=None,thickness=None,edgethickness=None):
        ...
        edgecolor = self.SafeColor(edgecolor)
        # *Q* OrientCoord() to do.
        cv2.drawPoly(self.image_buffer, np.array([pointlist]), color=edgecolor,thickness=int(thickness + (2 * edgethickness)))
        cv2.drawPoly(self.image_buffer, np.array([pointlist]), color=color,thickness=thickness)
        self.action_list.append(['drawedgepolygon',pointlist,color,edgecolor,thickness,edgethickness])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def FillRectangle(self,startcoord,endcoord,color=None):
        ...
        startcoord = self.orient_coord(startcoord) # Get height right way up.
        endcoord = self.orient_coord(endcoord) # Get height right way up.
        self.image_buffer = cv2.rectangle(self.image_buffer, startcoord, endcoord, color, thickness=-1)
    ...
    def FadeRectangle(self,startcoord,endcoord,color=None,fadecolor=None):
        ...
        for i in range(rad, 0, -4):
            ...
            if gradedcolor != prevcolor:
                ...
                startcoord = self.orient_coord(startcoord) # Get height right way up.
                endcoord = self.orient_coord(endcoord) # Get height right way up.
                self.image_buffer = cv2.rectangle(self.image_buffer, startcoord, endcoord, gradedcolor, thickness=-1)
                prevcolor = gradedcolor
        self.action_list.append(['faderectangle',startcoord,endcoord,color,fadecolor])
        self.modified_timestamp = self.NowUTC()
        return True

    def draw_ellipse(self,center_x, center_y, axis_x, axis_y, angle=0, startAngle=0, endAngle=360, color=None, thickness=None):
        ...
        center_y = self.orient_height(center_y) # Does height increase from the TOP or BOTTOM of the image?
        # *Q* OrientHeight needs to switch angles too.
        self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (int(axis_x), int(axis_y)), int(angle), int(startAngle), int(endAngle), color, thickness, lineType=cv2.LINE_AA)
        self.action_list.append(['drawellipse',center_x,center_y,axis_x,axis_y,angle,startAngle,endAngle,color,thickness])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def DrawEdgeEllipse(self,center_x, center_y, axis_x, axis_y, angle=0, startAngle=0, endAngle=360, color=None, edgecolor=None, thickness=None, edgethickness=1):
        ...
        center_y = self.orient_height(center_y) # Does height increase from the TOP or BOTTOM of the image?
        # *Q* OrientHeight needs to switch angles too.
        self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (int(axis_x), int(axis_y)), int(angle), int(startAngle), int(endAngle), edgecolor, (thickness + 2 * edgethickness), lineType=cv2.LINE_AA)
        self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (int(axis_x), int(axis_y)), int(angle), int(startAngle), int(endAngle), color, thickness, lineType=cv2.LINE_AA)
        self.action_list.append(['drawedgeellipse',center_x,center_y,axis_x,axis_y,angle,startAngle,endAngle,color,thickness])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def FillEllipse(self,center_x, center_y, axis_x, axis_y, angle=0, startAngle=0, endAngle=360, color=None):
        ...
        center_y = self.orient_height(center_y) # Does height increase from the TOP or BOTTOM of the image?
        # Weird behaviour of this function in Python3 (See online) - parameters very fussy about grouping and datatype.
        self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (int(axis_x), int(axis_y)), int(angle), int(startAngle), int(endAngle), color, thickness=-1, lineType=cv2.LINE_AA)
        self.action_list.append(['fillellipse', center_x, center_y, axis_x, axis_y, angle, startAngle, endAngle, color])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def FillEllipse2(self,center_x, center_y, axis_x, axis_y, angle=0, startAngle=0, endAngle=360, color=None):
        ...
        if inframe: # Filling an ellipse can be slow, so only do it if the object is likely to be in bounds.
            # Weird behaviour of this function in Python3 (See online) - parameters very fussy about grouping and datatype.
            self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (int(axis_x), int(axis_y)), int(angle), int(startAngle), int(endAngle), color, thickness=-1, lineType=cv2.LINE_AA)
        self.action_list.append(['fillellipse2', center_x, center_y, axis_x, axis_y, angle, startAngle, endAngle, color, left, right, bottom, top, width, height])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def FadeEllipse(self,center_x, center_y, axis_x, axis_y, angle=0, startAngle=0, endAngle=360, color=None, fadecolor=None):
        ...
        for i in range(rad, 0, -2):
            ...
            if gradedcolor != prevcolor:
                ...
                center_y = self.orient_height(center_y) # Does height increase from the TOP or BOTTOM of the image?
                self.image_buffer = cv2.ellipse(self.image_buffer, (int(center_x), int(center_y)), (xt, yt), int(angle), int(startAngle), int(endAngle), gradedcolor, thickness=-1, lineType=cv2.LINE_AA)
                prevcolor = gradedcolor
        self.action_list.append(['fadeellipse', center_x, center_y, axis_x, axis_y, angle, startAngle, endAngle, color, fadecolor])
        self.modified_timestamp = self.NowUTC()
        return True
    ...
    def DrawCrosshairs(self,x,y,radius,style=255,outerradius=None,spoke_count=4,spoke_angle=0,color=None,thickness=None,arrowpixels=None):
        ...
        if style & PilomarImage.CROSSHAIR_DOT: # Draw dot in the centre of the crosshairs.
            self.draw_circle(center_x=x,center_y=y,rad=2,color=color,thickness=-1) # Filled.
        ...
        if style & PilomarImage.CROSSHAIR_SPOKES: # Draw vertical and horizontal lines of the crosshairs.
            for i in range(spoke_count): # Process each spoke in turn.
                ...
                if i == 0: self.draw_line(start_point,end_point,color=color,thickness=thickness,arrowpixels=arrowpixels) # Draw spoke with optional direction arrow head.
                else: self.draw_line(start_point,end_point,color=color,thickness=thickness,arrowpixels=None) # Draw spoke without direction arrow head.
        return False
    ...
    def DrawDumbbell(self,drawfrom,drawto,rad,fromcolor,tocolor,linecolor,arrow,thickness=None):
        ...
        if distance > dia: # Enough space to draw a line.
            ...
            if arrow: self.image_buffer = cv2.arrowedLine(self.image_buffer, (startx,starty), (endx,endy), linecolor, thickness=thickness, line_type=cv2.LINE_AA, tipLength=arrowproportion)
            else: self.image_buffer = cv2.line(self.image_buffer, (startx,starty), (endx,endy), linecolor, thickness=thickness, lineType=cv2.LINE_AA)
        self.image_buffer = cv2.circle(self.image_buffer,(fromx, fromy), rad, fromcolor, thickness=thickness, lineType=cv2.LINE_AA)
        self.image_buffer = cv2.circle(self.image_buffer,(tox, toy), rad, tocolor, thickness=thickness, lineType=cv2.LINE_AA)
        self.action_list.append(['drawdumbbell',drawfrom,drawto,rad,fromcolor,tocolor,linecolor,arrow,thickness])
        self.modified_timestamp = self.NowUTC()
    ...
    def DescribeImage(self):
        ...
        print("Created:",self.created_timestamp,
