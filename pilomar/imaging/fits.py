...
def color_gain(
    array: np.ndarray, red: float = 1.0, green: float = 1.0, blue: float = 1.0
) -> np.ndarray:
...
def parse_args(arg_list: list[str], arg_dict: dict | None = None) -> dict[str, dict]:
...
class FitsCapture:
    ...
    def __init__(self, logger: Any = None, verbose: bool = False, debug: bool = False):
    ...
    def build_controls(
        self, arguments: dict[str, dict], shutter_specified: bool = False
    ) -> dict[str, Any]:
    ...
    def _save_fits(
        ...
        tags["MIDPOINT"] = {
            "value": capture_midpoint.isoformat(),
            "comment": "Midpoint of exposure",
        }
