...
class Hardware(AttributeMaster):
    ...
    def __init__(self, os_cmd, input_pin_factory=None, logger=None):
        ...
        self.log(
            f"Hardware.__init__(): \n\t"
            f"RPi: Model: {self.rpi_model}\n\t"
            f"Num: {self.rpi_num} \n\t"
            f"OStype: {self.os_type}\n\t"
            f"OSid: {self.os_version_id}\n\t"
            f"OSname: {self.os_version_name}\n\t"
            f"OSbits: {self.os_bits}\n\t"
            f"OSproc: {self.os_proc}\n\t"
            f"SysKey: {self.os_systemkey}",
            terminal=False,
        )
        self.tiny_flag_pin = None
        self.usb_protect_flag_pin = None
    ...
    def identify_pcb(self, autodetect=True):
        ...
        if autodetect and self._input_pin_factory:
            self.log(
                f"Hardware.identify_pcb(): Begin. TinyFlagPin: {self.tiny_flag_pin}, "
                f"UsbProtectFlagPin: {self.usb_protect_flag_pin}",
                terminal=False,
            )
            ...
            self.log(
                "Hardware.identify_pcb(): Skipped (no GPIO or autodetect=False)",
                terminal=False,
            )
            self.pcb_mctl_family = "tiny"
            self.pcb_protect_usb = True
        ...
        self.log(
            f"Hardware.identify_pcb(): Family: {self.pcb_mctl_family}, "
            f"UsbProtect: {self.pcb_protect_usb}, "
            f"pcb revision: {self.pcb_revision}",
            terminal=False,
        )
    ...
    def native_camera_driver(self):
        ...
        self.log(
            ...
            "O/S found, assuming camera driver is",
            self.camera_driver,
            terminal=False,
        )
    ...
    def hardware_is_supported(self, allow_fail=True):
        ...
        if self.os_systemkey in Hardware.SUPPORTED_SYSTEMS:
            ...
            if allow_fail:
                raise RuntimeError(
                    f"{self.program_title} is not designed to run "
