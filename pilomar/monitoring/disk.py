...
from pilomar.ui.text_color import TextColor # Text interface color utility.
import os
...
class DiskMonitor(): # 2 references.
    ...
    def __init__(self,name='root',devname='/dev/root',path='/',disctype='boot',logger=None):
        ...
        self.disk_type = disctype # 'boot' or 'usb'. 'usb' triggers some extra processing to check it is mounted and available.
        ...
        self.df_path = path # This is the path including the device label (ie /media/pi/USBMEMORY   )
        # USB storage details.
        self.usb_scan_list = ['/dev/sda1','/dev/sdb1'] # Which devices will be considered?
        self.usb_alarm_labels = ['CIRCUITPY'] # Raise an alarm if we see these devices connected.
        self.usb_label = None # Label of the device when it was formatted. Something like 'USBMEMORY'. Will appear in DfPath and is part of drive mapping.
        self.usb_uuid = None # Unique ID
        self.usb_type = None # file system type. FAT/FAT32.
        self.usb_part_uuid = None # Unique ID
        if devname != None:
            self.drive_available = True # True if available (eg root or USB is mounted), else False.
            if self.disk_type in ['usb']: # USB devices may need mounting. Check 'em out!
                self.FindUSB(devname=self.dev_name) # Check if USB memory stick is available.
        else:
            self.drive_available = False # There is no drive.
        self.poll(force=True) # Kickstart the values.
        if self.Log != None: self.Log("discmonitor: Available storage on:",self.Name, self.dev_name, self.Path, self.disk_free,'bytes',terminal=False)
    ...
    def Poll(self,force = False):
        ...
        if force or self.timer.Due():
            ...
            if self.drive_available: # Drive is available, so report the space left.
    ...
    def FreeMegaBytes(self,force=False) -> int:
        ...
        return self.free_bytes(force=force) / (1024 ** 2)
        
    def FreeGigaBytes(self,force=False) -> int:
        """ Return the amount of memory left free in megabytes. """
        return self.free_bytes(force=force) / (1024 ** 3)
    ...
    def DiscOK(self, force = False) -> bool:
        ...
        if megabytes < self.low_disk_mb: return False
        else: return True
    ...
    def FindUSB(self,devname='/dev/sda1'):
        ...
        if devname in self.usb_scan_list: # Safety check. Don't run commands with values we don't trust.
            ...
            for line in lines: # Run through each result line in turn.
                ...
                if items[0][:-1] == devname: # Found the USB memory stick. Extract details. (Ignore trailing ':' character)
                    self.usb_label = items[1].split("=")[1].replace('"','') # Volume label - Will be the folder name under /media/pi/{USBLabel}
                    self.usb_uuid = items[2].split("=")[1].replace('"','') # Unique ID of the memory stick.
                    ...
                    if self.Log != None: self.Log("discmonitor.FindUSB: Device:",devname,"Label:",self.usb_label,"UUID:",self.usb_uuid,"FS Type:",self.USBFSType,"PARTUUID:",self.USBPartUUID,terminal=False)
        ...
        if result: # Previous steps succeeded.
            if self.Log != None: self.Log("discmonitor.FindUSB:",devname,"is recognised as",self.usb_label,".",terminal=False)
            if ' ' in self.usb_label: # There's a space in the label name. Refuse to mount it.
                textline = "discmonitor.FindUSB: Media label '" + self.usb_label + "' contains spaces, will not mount. Please rename the media."
                if self.Log != None: self.Log(textline,level='error',terminal=True)
                textcolor.TextBox(textline,fg=textcolor.RED,bg=textcolor.BLACK)
                result = False 
        ...
        if result: # OK so far.
            self.df_path = self.Path + "/" + self.usb_label # The path to the mapped drive as it will appear in 'df' command output and in directory structures later on.
            if self.Log != None: self.Log("discmonitor.FindUSB: Checking if",devname,"is mounted as",self.df_path,terminal=False)
            if os.path.exists(self.df_path): # The directory exists. (Risk that this is a real directory on the SD card! See error warning later!)
                if self.Log != None: self.Log("discmonitor.FindUSB:",self.df_path,"exists.",terminal=False)
            else: # The directory does not exist. The drive is recognised by the system, but not mounted. Try to mount it now.
                if self.Log != None: self.Log("discmonitor.FindUSB:",self.df_path,"does not exist. Will attempt to mount.",terminal=True)
                # Warn the user that the 'pi' user password will be required. The udisksctl utility requires it in order to mount the disc.
                lines = ['Mounting ' + self.usb_label + ' under ' + self.Path]
                ...
                temp = self.os_cmdCode(cCmd) # Check return code.
                if temp == 0: # Return code '0' means success.
                    print('Thank you.')
                    if self.Log != None: self.Log("discmonitor.FindUSB: Mount",devname,"as",self.df_path,'success.',terminal=False)
                else: # Any other return code value means a problem.
                    result = False # Failed.
                    if self.Log != None: self.Log("discmonitor.FindUSB: Mount",devname,"as",self.df_path,'failed. (code',temp,')',level='error',terminal=True)
        if result: # OK so far.
            dictionary = self.GetDfDictionary() # Get the 'df' results from the operating system.
            if self.df_path in dictionary: # We found it now in the list of mount points.
                if self.Log != None: self.Log("discmonitor.FindUSB: Check",devname,"as",self.df_path,'found in df listing.',terminal=False)
            else: # We still can't find it. Something failed.
                if self.Log != None: 
                    self.Log("discmonitor.FindUSB: Check",devname,"as",self.df_path,'not found in df listing.',level='error',terminal=True)
                    self.Log("discmonitor.FindUSB: Troubleshooting: Check for rogue " + self.df_path + " folder on SD card.",terminal=False)
                lines = ["Troubleshooting",
                         "If the USB drive is installed but does not appear in the .df listing:",
                         "There may be a false folder named " + self.df_path + " on the SD card which prevents mounting the USB memory.",
                         "To solve :-",
                         "- Delete the false USB folder " + self.df_path,
        ...
        if self.Log != None: self.Log("discmonitor.FindUSB: DriveAvailable",self.drive_available,terminal=False)
        return result
