...
from pilomar.core.timer import Timer, ProgressTimer # Pilomar's timer classes.
...
from pilomar.imaging.keogram import PilomarKeogram # Pilomar's keogram builder

# UI imports (optional - may not be available yet)
try:
    from pilomar.ui.text_color import TextColor
    from pilomar.ui.keyboard import KeyboardScanner
except ImportError:
    TextColor = None
    KeyboardScanner = None
...
class AstroLens():
    ...
    def __init__(self,length,horizontal_fov,vertical_fov,aperture=2.8,multiplier=5.6,logger=None,parameters=None):
        ...
        self.equiv_length = self.length * self.equiv_multiplier # From https://www.seeedstudio.com/blog/2020/06/18/a-complete-guide-to-help-you-choose-lenses-for-your-raspberry-pi-high-quality-camera-m/ 35mm equivalent focal length (?) (AKA the Crop Factor for the sensor?
        self.fov_horizontal = horizontal_fov
        self.fov_vertical = vertical_fov
        self.fov = min(self.fov_horizontal, self.fov_vertical) # When calculating the FOV for a survey, use the smaller value.
        self.aperture = aperture # FStop of the lens. *Q* Multiplier will impact this too. Hmm...
        #self.ID = str(self.length) + "|" + str(self.fov_horizontal) + "|" + str(self.fov_vertical) # Unique ID of lens features.
        self.log("AstroLens: Length:",str(self.length),"mm (equiv.",str(self.equiv_length),"mm) FoV:",
                 str(self.fov_horizontal),"deg","*",str(self.fov_vertical),"deg",terminal=False)
        astrolens.lens_list.append(self) # Add this instance to the global list of all defined lenses.
    ...
    def SetLogger(self,logger):
        ...
        self.report_exception = self._NullLogger # Cannot report exception details to logfile.
        self.raise_exception = self._NullLogger # Cannor report and raise exception. 
        if hasattr(logger,'Log'): self.log = logger.log # Log method.
        if hasattr(logger,'ReportException'): self.report_exception = logger.report_exception # Report exception details to logfile.
        if hasattr(logger,'RaiseException'): self.raise_exception = logger.raise_exception # Report and raise exception. 
    ...
    def EstimateFoV(self,length):
        ...
        if lower_entry != None: # We found a reasonable match.
            ...
            if upper_entry != None and self.equiv_length != lower_entry[0]:
        ...
        self.log("astrolens.EstimateFoV():",self.equiv_length,self.fov_horizontal,self.fov_vertical,terminal=False)
...
class AstroSensor():
    ...
    def HmsFromStamp(self,timestamp,dateaware=False):
        ...
        try:
            if timestamp is None: # Protect from null values.
                ...
                if dateaware and timestamp.date() != self._now_utc().date(): # The date is not today.
    ...
    def NowHMS(self):
        ...
        return self.HmsFromStamp(self._now_utc())
    ...
    def _SetMode(self,mode : int):
        ...
        if self.camera_window != None: self.camera_window.Print("Sensor mode: " + str(mode))
    ...
    def DisableCleanup(self):
        ...
        if self.parameters.CameraDriver == 'raspistill': #if OS_name in ['buster']:
            ...
            if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " On Chip Cleanup - OFF")
    ...
    def EnableCleanup(self):
        ...
        if self.parameters.CameraDriver == 'raspistill': #if OS_name in ['buster']:
            ...
            if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " On Chip Cleanup - ON")
        else: # libcamera has a command line option to disable cleanup.
...
class AstroCamera():
    ...
    def __init__(self,inp_sensor,inp_lens,exposure=10.0,trackingexposure=5.0,logger=None,parameters=None,imagesimulator=None):
        ...
        self.error_window = None # Can declare an error window (textcolor library) to copy error messages to.
        self.camera_window = None # Can declare a camera window (textcolor library) to copy camera events to.
    ...
    def SetObservationParameters(self):
        ...
        if self.ObjectType in ['aurora','meteor']: # Don't generate preview images for meteors or aurora.
            if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " No preview's generated for " + self.ObjectType + " recordings.")
            ...
            if self.camera_window != None: self.Print(self.NowHMS() + " Preview generation is disabled in parameters.")
        ...
        if self.ObjectType in ['meteor','altaz','earth satellite','aurora']: # No need to perform drift tracking for these targets.
            ...
            if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " No tracking performed for " + self.ObjectType + " targets.")
    ...
    def CalibrateFov(self):
        ...
        self.Lens.fov_horizontal = round(self.Sensor.PixelWidth * MoonMeanDiaDeg / moonpixels,1) # FOV to 1 decimal place is enough.
        self.Lens.fov_vertical = round(self.Sensor.PixelHeight * MoonMeanDiaDeg / moonpixels,1) # FOV to 1 decimal place is enough.
        self.Lens.fov = min(self.Lens.fov_horizontal, self.Lens.fov_vertical) # When calculating the FOV for a survey, use the smaller value.
        self.log("astrocamera.CalibrateFov: Chosen H.FoV:",self.Lens.fov_horizontal,"V.FoV:",self.Lens.fov_vertical,terminal=False)
        ...
        if result: # Make these changes permanent by setting them in the parameter file.
            ...
            self.parameters.LensHorizontalFov = self.Lens.fov_horizontal
            self.parameters.LensVerticalFov = self.Lens.fov_vertical
    ...
    def Reset(self):
        ...
        self.BatchCount = 0 # How many photos taken in the current observation batch?
        self.BatchData = {} # Dictionary of captured images and metadata.
        if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " astrocamera.Reset")
    ...
    def CaptureStartAge(self):
        ...
        if self.CaptureStart != None:
            result = self._now_utc() - self.CaptureStart
        return result
    ...
    def LastImageAge(self):
        ...
        if self.LastImageDateTime != None:
            result = self._now_utc() - self.LastImageDateTime
        return result
        ...
        if self.parameters.CameraEnabled and self.CaptureStart != None: # An attempt has been made and the camera is on!
            if self.CaptureEnd is None or self.CaptureEnd <= self.CaptureStart: # It hasn't completed yet.
                ...
                if self.CaptureStartAgeSeconds() > faultdelay: # It has been running too long.
                    ...
                    if self.camera_window != None: self.camera_window.Print(self.NowHMS() + ' ' + line)
    ...
    def ModeChange(self):
        ...
        self.PixelsPerFovDegreeWidth = int(self.Sensor.PixelWidth / self.Lens.fov_horizontal) # Conversion value between ANGLE and PIXELS.
        self.PixelsPerFovDegreeHeight = int(self.Sensor.PixelHeight / self.Lens.fov_vertical) # Conversion value between ANGLE and PIXELS.
        ...
        ArcSecondsPerWidth = self.Lens.fov_horizontal * 60 * 60
    ...
    def CaptureSetFull(self,file_root,batch_size,camera_command,tempfile=False,terminal=True,cleanup=True,astrotime=None):
        ...
        for i in range(batch_size):
            ...
            self.CaptureStart = self._now_utc()
            if self.parameters.CameraEnabled: # Camera is enabled. Take real photo.
                self.os_cmd(cmd,output='none')
                retc = self.os_command.ReturnCode # What did the camera command exit with ?
                self.log("astrocamera.CaptureSetFull(): Return code:",retc,terminal=False)
                if retc != 0: # Non zero return code. Did something go wrong?
                    if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " Return code " + str(retc),fg=textcolor.YELLOW)
                    if self.error_window != None: self.error_window.Print(self.NowHMS() + " capture returned code " + str(retc),fg=textcolor.YELLOW)
            ...
            if self.Mctl != None: # Microcontroller handle is known.
                if remoterestarts != self.Mctl.RemoteRestarts: # Microcontroller reset during exposure.
                    ...
                    if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " Motors reset during exposure.") # Just the filename.
                    if self.error_window != None: self.error_window.Print(self.NowHMS() + " Motors reset during exposure.") # Just the filename.
                    rejectimage = True # We should reject this image.
            if not tempfile: # Display the filename if it's permanent, ignore the tempfile.
                if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " " + outputfile.split('/')[-1]) # Just the jpg filename.
            ...
            self.LastImageDateTime = self._now_utc() # *Q* This timestamp is AFTER the image has been captured. Can it be estimated better? CaptureStart + (CaptureEnd - CaptureStart) / 2 ?
            ...
            if (" -r " in (camera_command + ' ') or " --raw " in (camera_command + ' ')) and self.parameters.CameraEnabled: # The jpg contains RAW data in the file tags, extract it. Convert it.
                if self.parameters.CameraDriver == 'raspistill': # We need to manually extract the DNG data.
                    ...
                    try:
                        ...
                        self.report_exception(e,comment='astrocamera.CaptureSetFull(): PiDNG.convert failed.')
                    ...
                    if not self.CameraSaveDng: # We don't need to keep the .dng file anymore.
                        ...
                        if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " " + dngname.split('/')[-1]) # Just the dng filename.
                    ...
                    if self.camera_window != None: self.camera_window.Print(self.NowHMS() + " " + fitsname.split('/')[-1]) # Just the dng filename.
            ...
            if batch_size > 1:
                dt_now = self._now_utc() # Current time.
    ...
    def BuildKeogram(self,altitude=None,azimuth=None):
        ...
        if len(allfiles) > 0:
            ...
            # Add altitude scale
            BaseAlt = altitude - (self.Lens.fov_vertical / 2) # What altitude does the bottom of the image represent?
            TopAlt = altitude + (self.Lens.fov_vertical / 2) # What altitude does the top of the image represent?
                ...
                if tempimage.ImageMissing(): # imread failed.
                    ...
                    if self.CameraSaveDng: # We don't need to keep the .dng file anymore.
                        try:
                            ...
                            CamLog.report_exception(e,comment='astrocamera.ProcessImageFiles() error when converting to DNG file.')
    ...
    def AutoPhoto(self):
        ...
        while inp != 'x':
            ...
            if inp == "x":
                print ("quit")
                break
            dt = self.CleanDatetimeString(str(self._now_utc()))
