...
def _log(ctx: TargetSelectionContext, *args, level: str = 'info', terminal: bool = False):
    """Log a message if logger is available."""
    if ctx.logger:
        ctx.logger.Log(*args, level=level, terminal=terminal)
...
class TargetChooser:
    ...
    def _log(self, *args, level: str = 'info', terminal: bool = False):
        """Log a message."""
        _log(self.ctx, *args, level=level, terminal=terminal)
    ...
    def choose_messier(
        ...
        while result == "":
            ...
            if search == "?":
                self._print("Recognised catalog IDs:", self.ctx.messier_numlist)
                self._print("Recognised names:", self.ctx.messier_namelist)
                continue
            ...
            for key, value in self.ctx.messier.items():
                if key.lower() == search:
                    result = key.lower()
                    self._log("ChooseMessier: Found by catalog number", key, terminal=False)
                    break
                name = value.get('name', '')
                if name[:len(search)].lower() == search:
                    result = key.lower()
                    self._log("ChooseMessier: Found by name", name, "catalog", key, terminal=False)
                    break
            
            if result == "":
                if prechosen is not None:
                    self._log("ChooseMessier: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
                self._print("Messier: Nothing matched, try again.")
        
        # Build target info
        entry = self.ctx.messier[result.upper()] if result.upper() in self.ctx.messier else self.ctx.messier.get(result, {})
        ...
        if width or height:
            ...
            if size_warning and self.ctx.camera:
                ...
                if size_pix < 10:
                    self._log("Target is quite small, about", int(size_pix), "pixels", terminal=True)
        ...
        while result == "":
            ...
            for name in self.ctx.ngc_namelist:
                if name == search:
                    ...
                    self._log("ChooseNGC: Found", name, terminal=False)
                    break
            
            if result == "":
                if prechosen is not None:
                    self._log("ChooseNGC: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
                self._print("NGC:", search, "Nothing matched, try again.")
    ...
    def choose_hipparcos(self, prechosen: Optional[str] = None) -> Optional[Dict]:
        ...
        while result == "":
            ...
            if hip_num is not None and hip_num in self.ctx.hipparcos_df.index:
                ...
                self._log("ChooseHipparcos: Found", result, terminal=False)
            
            if result == "":
                if prechosen is not None:
                    self._log("ChooseHipparcos: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
        ...
        while result == "":
            ...
            for designation in self.ctx.comets_df['designation']:
                if designation == search:
                    ...
                    self._log("ChooseComet: Found", designation, terminal=False)
                    break
            
            if result == "":
                if prechosen is not None:
                    self._log("ChooseComet: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
    ...
    def choose_meteor(self, prechosen: Optional[str] = None) -> Optional[Dict]:
        ...
        while result == "":
            ...
            for name, value in self.ctx.meteors.items():
                if name == search:
                    result = name.lower()
                    self._log("ChooseMeteor: Found", name, terminal=False)
                    break
            
            if result == "":
                if prechosen is not None:
                    self._log("ChooseMeteor: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
                self._print("Meteor: Nothing matched, try again.")
        
        entry = self.ctx.meteors[search]
        
        # Create initial star position for radiant
        radiant = _create_star(
            (entry['rah'], entry['ram'], 0),
            (entry['dec'], 0, 0)
        )
    ...
    def choose_solar(self, prechosen: Optional[str] = None) -> Optional[Dict]:
        ...
        while result == "":
            ...
            if search in targets:
                ...
                if prechosen is not None:
                    self._log("ChooseSolar: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
    ...
    def choose_satellite(self, prechosen: Optional[str] = None) -> Optional[Dict]:
        ...
        while result == "":
            ...
            if search in self.ctx.satellite_list:
                result = search
                self._log("ChooseSatellite: Found", result, terminal=False)
            else:
                if prechosen is not None:
                    self._log("ChooseSatellite: Prechosen", prechosen, "not recognised", terminal=False)
                    return None
                self._print(f"'{search}' is not recognised. Try again.")
        ...
        while True:
            ...
            try:
                ...
                if ded is None or not (-90 <= ded <= 90):
                    ...
                    continue
                if dem is None or not (-60 < dem < 60):
    ...
    def choose_aurora(self, prechosen: Optional[str] = None) -> Optional[Dict]:
