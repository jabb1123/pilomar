...
try:
    ...
    from observation import ObservationContext, ObservationLoop, start_observation
    from data_loader import CatalogLoader, CatalogPaths
    # Try to import ListChooser from ui module
    try:
...
class ApplicationContext:
    ...
    # Satellite data
    celestrak: Any = None  # CelesTrak satellite TLE handler
...
class Application:
    ...
    def _init_target_chooser(self) -> None:
        ...
        messier_namelist = [v.get('name', '') for v in self.ctx.messier_dict.values()] if self.ctx.messier_dict else []
        ...
        comet_list = list(self.ctx.comets_df['designation']) if self.ctx.comets_df is not None else []
        ...
        if self.ctx.parameters:
            home_latitude = getattr(self.ctx.parameters, 'HomeLat', 
    ...
    def _load_parameters(self) -> None:
        ...
        try:
            ...
            pilomar_parent = os.path.dirname(os.path.dirname(
    ...
    def _prompt_for_location(self) -> None:
        ...
        # Set the parameters
        self.ctx.parameters.HomeLat = lat_str
        self.ctx.parameters.HomeLon = lon_str
        self.ctx.parameters._HomeLatVal = lat_val
        self.ctx.parameters._HomeLonVal = lon_val
    ...
    def _init_skyfield(self) -> None:
        ...
        try:
            ...
            self.ctx.skyfield_loader = load
            
            # Load timescale
            self._log("Loading Skyfield timescale...", terminal=False)
            self.ctx.timescale = load.timescale()
            ...
            self._log("Skyfield not available - some features disabled", 
    ...
    def _load_catalogs(self) -> None:
        ...
        loader = CatalogLoader(
            paths, 
            logger=self.ctx.main_log,
            skyfield_loader=self.ctx.skyfield_loader
        )
        ...
        try:
            ...
            if self.ctx.hipparcos_df is not None:
                self._log(f"Loaded {len(self.ctx.hipparcos_df)} Hipparcos stars", terminal=False)
        except Exception as e:
    ...
    def _load_celestrak(self) -> None:
        ...
        try:
            ...
            class LogAdapter:
                ...
                def ReportException(self, e, msg):
                    self._log(f"{msg}: {e}", level='error')
                def RaiseException(self, e, msg):
                    self._log(f"{msg}: {e}", level='error')
                    raise e
            ...
            self.ctx.celestrak = Celestrak(
                celestrak_url, 
                logger=logger,
                projectroot=self.ctx.project_root
            )
            
            sat_count = len(self.ctx.celestrak.SatelliteList) if self.ctx.celestrak else 0
    ...
    def _goto_target(self) -> None:
        ...
        target_name = self.ctx.target.get('name', 'Unknown') if isinstance(self.ctx.target, dict) else str(self.ctx.target)
    ...
    def _select_messier(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('messier'):
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name, 'searchgroup': 'messier', 'searchterm': name}
    ...
    def _select_ngc(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('ngc'):
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name, 'searchgroup': 'ngc', 'searchterm': name}
                print(f"Selected target: {name}")
    ...
    def _select_star(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('hipparcos'):
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name, 'searchgroup': 'hipparcos', 'searchterm': name}
                print(f"Selected target: {name}")
    ...
    def _select_solar_system(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('solar'):
            ...
            try:
                idx = int(choice) - 1
                if 0 <= idx < len(bodies):
                    self.ctx.target = {'name': bodies[idx], 'searchgroup': 'solar', 'searchterm': bodies[idx].lower()}
    ...
    def _select_comet(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('comets'):
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name, 'searchgroup': 'comet', 'searchterm': name}
                print(f"Selected target: {name}")
    ...
    def _select_meteor(self) -> None:
        ...
        if self._target_chooser and self._has_catalog_data('meteors'):
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name, 'objecttype': 'meteor', 'searchgroup': 'meteor', 'searchterm': name}
    ...
    def _select_satellite(self) -> None:
        ...
        if self._target_chooser:
            ...
            if name and name.lower() != 'x':
                self.ctx.target = {'name': name.upper(), 'searchgroup': 'satellite', 'searchterm': name.lower()}
    ...
    def _select_radec(self) -> None:
        ...
        if self._target_chooser:
            ...
            try:
                ...
                self.ctx.target = {'name': f"RA{ra}_Dec{dec}", 'searchgroup': 'radec', 'searchterm': f"{ra},{dec}"}
    ...
    def _select_altaz(self) -> None:
        ...
        if self._target_chooser:
            ...
            try:
                ...
                self.ctx.target = {'name': f"Alt{alt}_Az{az}", 'objecttype': 'altaz', 'searchgroup': 'altaz', 'searchterm': f"{alt},{az}"}
    ...
    def _select_aurora(self) -> None:
        ...
        if self._target_chooser:
            ...
            if not alt:
                alt = '30'
            self.ctx.target = {'name': 'Aurora', 'objecttype': 'aurora', 'searchgroup': 'aurora', 'searchterm': f"{alt},{az}"}
    ...
    def _begin_observation(self) -> None:
        ...
        target_name = self.ctx.target.get('name', self.ctx.target) if isinstance(self.ctx.target, dict) else str(self.ctx.target)
    ...
    def _set_exposure_time(self) -> None:
        ...
        if self.ctx.camera:
            ...
            current = getattr(self.ctx.parameters, 'ExposureSeconds', 1.0) if self.ctx.parameters else 1.0
    ...
    def _move_altitude(self) -> None:
        ...
        max_alt = getattr(self.ctx.parameters, 'MaxAltitudeAngle', 90) if self.ctx.parameters else 90
        ...
        if self.ctx.target:
            ...
            try:
                ...
                if exposure:
                    ...
                    exposure = getattr(self.ctx.parameters, 'ExposureSeconds', 1.0) if self.ctx.parameters else 1.0
                
                # Add to schedule
                entry = {
                    'target': dict(self.ctx.target) if isinstance(self.ctx.target, dict) else {'name': str(self.ctx.target)},
                    'frames': frames,
                    'exposure': exposure,
                }
    ...
    def _offer_home_camera(self) -> None:
        ...
        try:
            response = input("Would you like to home the camera before powering off? [y/N] ").lower().strip()
