...
class OsCommand:
    ...
    def set_logger(self, logger):
        ...
        Args:
            logger: Logger instance or Log method
        """
        self.log = logger
    ...
    def execute(self, cmd: str, output: str = 'none') -> List[str]:
        ...
        try:
            ...
            if self.log is not None:
                ...
                self.log(f"OsCommand.execute({cmd}) returned returncode {e.returncode}", 
                        terminal=False)
                self.log(f"OsCommand.execute({cmd}) returned output {e.output}", 
                        terminal=False)
                self.log(f"OsCommand.execute({cmd}) returned cmd {e.cmd}", terminal=False)
                self.log(f"OsCommand.execute({cmd}) returned stdout {e.stdout}", 
                        terminal=False)
                self.log(f"OsCommand.execute({cmd}) returned stderr {e.stderr}", 
                        terminal=False)
            result = ""
    ...
    def execute_code(self, cmd: str, output: str = 'none') -> int:
        ...
        try:
            ...
            if self.log is not None:
                ...
                self.log(f"OsCommand.execute_code({cmd}) returned returncode {e.returncode}", 
                        terminal=False)
                self.log(f"OsCommand.execute_code({cmd}) returned output {e.output}", 
                        terminal=False)
                self.log(f"OsCommand.execute_code({cmd}) returned cmd {e.cmd}", 
                        terminal=False)
                self.log(f"OsCommand.execute_code({cmd}) returned stdout {e.stdout}", 
                        terminal=False)
                self.log(f"OsCommand.execute_code({cmd}) returned stderr {e.stderr}", 
                        terminal=False)
            returncode = e.returncode
            result = ""
