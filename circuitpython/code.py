...
ACCEPTABLERPIVERSIONS = [
    "1.0"
]  # Which RPi versions are acceptable? (Ignore patch level)
...
Bootline = (
    ""  # Make sure the entire boot_out.txt content is available as a single item.
)
    ...
    while True:  # Loop through all the lines in turn.
        ...
        for item in lines:
            ...
            for elements in item.split(" "):  # Split into individual elements.
                if elements.strip().lower() == "circuitpython":
                    CircuitPython = True  # This is a CircuitPython build.
...
class statusled:
    ...
    def Task(self, task):
        ...
        )  # Seconds timer. Reduce by 1.0e9 to get seconds.
        if (
            ms < self.StatusExpiry
        ):  # Error code is still valid. No other setting can be used yet.
            pass
        elif (
            self.Enabled or task == ErrorTask
        ):  # No current ERROR status, so we can consider whether to set the new status.
            if (
                task == ErrorTask
            ):  # Setting a new error status. Set the timeout for 1 second ahead.
    ...
    def SetRGB(self, line):
        ...
        lli = len(lineitems)
        if lli > 3 and lineitems[3] == "y":
            self.LedR.On()
        else:
            self.LedR.Off()
        if lli > 4 and lineitems[4] == "y":
            self.LedG.On()
        else:
            self.LedG.Off()
        if lli > 5 and lineitems[5] == "y":
            self.LedB.On()
        else:
            self.LedB.Off()
        if lli > 6:
            self.StatusExpiry = int(time.monotonic_ns() / 1000000000) + int(
                lineitems[6]
            )  # Set expiry on the LED color.
...
class timer:
    ...
    def __init__(self, name, repeat, offset=0):
        ...
        self.Name = name
        if repeat < 1:
            repeat = 1  # Minimum repeat cycle is 1 second.
        self.RepeatSeconds = repeat
        if offset != 0:
            self.NextDue = (
                time.time() + offset
            )  # If the timer starts with an offset then that's the FIRST due time.
        else:
            self.NextDue = (
                time.time() + repeat
            )  # There's no offset, so due time is calculated from now.
    ...
    def SetNextDue(self):
        ...
        while self.NextDue <= time.time():
            gap = (
                (time.time() - self.NextDue) // self.RepeatSeconds
            ) + 1  # How many multiples of 'repeatseconds' do we need to add to get to the next timeslot?
            if gap >= 0:
    ...
    def Due(self):
        ...
        temp = self.Remaining()
        if (
            temp > self.RepeatSeconds
        ):  # If the local clock changes, the timer may be left out of sync. Reset it.
            print(
                "clock(",
                self.Name,
                ") remaining",
    ...
    def Log(self, line, *args):
        ...
        for x in args:
            if type(x) == type(str):
                ...
                a = str(x)  # Convert all additional items to strings and append them.
            line = (line + " " + a).strip()
        if self.Clock == None:
...
class uarthost:
    ...
    def __init__(self, name=None, channel=0):
        print(dir(board))
        if name == None:
            name = "UART" + str(channel)  # Default to useful name.
        if channel == 0:  # UART0
            self.uart = busio_UART(
                board.GP0,
                board.GP1,
                baudrate=115200,
                receiver_buffer_size=1024,
                timeout=0,
            )  # Define UART0 as the serial comms channel to the host.
            print("UART TX=", board.GP0, "UART RX=", board.GP1)
        else:  # UART1
            raise Exception(
                "uarthost on Tiny2040 not configued for UART channel 1. Use channel 0 only."
            )
        ...
        self.PicoRxErrors = (
            0  # How many checksum rejections occurred with received data?
        )
        ...
        self.ReadDrops = (
            0  # How many received messages are dropped because input buffer is full?
        )
        self.LastTxms = self.LastRxms = (
            self.ticks_ms()
        )  # Milliseconds since last transmission.
    ...
    def Reset(self):
        ...
        for i in range(2):
            self.Write(
                "#" * 20
            )  # Send dummy lines through the UART line to flush out any junk.
        self.Write("# CP env " + str(Bootline) + " ver " + str(CircuitPythonVersion))
        self.Write(
            "# CP mem alloc " + str(gc.mem_alloc()) + " free " + str(gc.mem_free())
        )
        self.Write("controller started")  # Tell the remote device we're up and running.
        self.Write(
            "controller version " + str(VERSION)
        )  # Tell the remote device which software version is running.

    def ticks_ms(self):
        """Standardise result of CircuitPython and MicroPython CPU ticks value."""
        return int(
            time.monotonic_ns() / 1000000
        )  # Nano seconds. Reduce by 1.0e6 to get milliseconds (as integer).
    ...
    def BufferInput(self):
        ...
        while self.RxWaiting():  # Input waiting in Rx buffer.
            ...
            if LoopCounter > 20:
                break  # Max 20 reads performed per call.
            try:
                ...
                LogFile.Log(
                    "uarthost.BufferInput: uart.read() or conversion error:", str(e)
                )
            ...
            if len(CharsToProcess) > 0:
                for cchar in CharsToProcess:
                    ...
                    if cchar == "\n":  # End of line
                        self.LinesRead += 1
                        if (
                            len(self.ReceivingLine) > 0
                            and self.ReceivingLine[-1] == "\n"
                        ):
                            line = (
                                self.ReceivingLine.strip()
                            )  # Clear special characters.
                            if len(line) > 0:  # Something to process.
                                if (
                                    len(self.ReceivedLines) < 10
                                ):  # Only buffer 10 lines, discard the rest. No space!
                                    self.ReceivedLines.append(
                                        line
                                    )  # Add to list of lines to handle.
                                    line = self.RemoveChecksum(line)
                                    report = "rec: " + line
                                    print(report)  # Report all receipts to serial out.
                                    if (
                                        line[0] != "#"
                                    ):  # Acknowledge receipt of all messages except comments via the log file back to the RPi too.
                                        x = line.split(" ")[
                                            -1
                                        ]  # Last entry should be message sequence number.
                                        if x.startswith("["):
                                            report = (
                                                "rec: " + x
                                            )  # If we have message sequence number, just report that back to the RPi.
                                        LogFile.Log(report)
                                else:
    ...
    def RemoveCounter(self, line):
        ...
        if temp >= 0:
            line = line[
                :temp
            ].strip()  # Strip anything after the last '[' character, it's a message count and not data.
        return line
    ...
    def Read(self):
        ...
        while len(line) == 0 and len(self.ReceivedLines) > 0:
            ...
            if self.ValidateChecksum(line):  # Checksum is good, trust the line.
                line = self.RemoveChecksum(line)
                line = self.RemoveCounter(
                    line
                )  # Strip any final message count from the end of the line.
    ...
    def WritePoll(self):
        ...
        StatusLed.Task("coms")
        if (
            len(self.WriteQueue[0]) > self.WriteChunk
        ):  # Pull max 20 chars from write queue.
    ...
    def Write(self, line, log=True):
        ...
        if len(line) > 0:
            while len(self.WriteQueue) >= 20:  # Only buffer 20 lines. Save memory.
                self.WriteQueue.pop(
                    1
                )  # Drop the 2nd entry, the first may already be partially transmitted.
                self.WriteDrops += 1
    ...
    def Heartbeat(self):
        ...
        self.Write(
            ...
            + IntToTimeString(Clock.Now())
            + " on "
            + IntToTimeString(time.time())
        )


RPi = uarthost(
    channel=0
)  # Create UART serial comms with Raspberry Pi. # Feather RP2040
...
class clock:
    ...
    def __init__(self, TimeValue=None):
        self.TimeDelta = (
            0  # Offset from machine clock to current date/time (in seconds).
        )
        self.ClockSynchronised = False  # Indicates that the clock is synchronised.
        self.PrevTime = (
            time.time()
        )  # Record the initial unmodified time of the clock. Used to detect if machine clock gets updated.
        ...
        self.FirstNS = float(
            time.monotonic_ns()
        )  # What's the nanosecond count at the start of the timestamp?
    ...
    def UpdateClockFromInt(self, TimeInt):
        ...
        result = False
        if (
            TimeInt != None and TimeInt > tn
        ):  # We can nudge the clock forward, never backwards.
            self.SetTimeFromInt(TimeInt)
            LogFile.Log(
                "UpdateClockFromInt(",
                IntToTimeString(TimeInt),
                ") replaces",
                IntToTimeString(tn),
                "Updated clock.",
            )
    ...
    def CheckTimeDelta(self, now):
        ...
        if now - self.PrevTime > 3600:  # Clock has suddenly jumped an hour or more!
            print(
                "c.CTD: Internal clock may have synchronised. Resetting clock synchronisation."
            )
            self.TimeDelta = 0
            self.ClockSynchronised = False
            LogFile.Log(
                "CheckTimeDelta(): Internal clock may have synchronised. Resetting clock synchronisation."
            )
        self.PrevTime = now
    ...
    def SetTimeFromInt(self, TimeInt):
        ...
        self.TimeDelta = max(
            TimeInt - time.time(), 0
        )  # Time offset is the received time - the realtime clock time. Cannot be negative.
        self.ClockSynchronised = True
        RPi.Write(
            "# Clock now "
            + IntToTimeString(self.Now())
            + " timedelta "
            + str(self.TimeDelta)
            + " seconds."
        )
    ...
    def SetTimeFromString(self, TimeString):
        ...
        try:
            ...
            LogFile.Log(
                "clock.SetTimeFromString: Invalid timestamp string (", TimeString, ")"
            )
            ExceptionCounter.Raise()  # Increment exception count for the session.
        return result
    ...
    def Now(self):
        ...
        self.CheckTimeDelta(
            now
        )  # If the internal clock has recently been synchronised clear TimeDelta until it can be recalculated.
    ...
    def NowDecimal(self):
        ...
        while CurrDecimal >= 1.0:  # Decimal must be within the current second.
            while (
                self.Now() != self.CurrTime
            ):  # If the clock has rolled on to a new second, calculate a ZERO point for the monotonic_ns() clock.
                self.FirstNS = float(
                    time.monotonic_ns()
                )  # This is the ZERO point for nanoseconds within this second.
                self.CurrTime = self.Now()  # The current clock time.
            CurrDecimal = (
                float(time.monotonic_ns()) - self.FirstNS
            ) / 1e9  # Elapsed fraction of a second since is started.
...
class trajectorypoint:
    ...
    def __init__(self, line):
        ...
        self.EndTime = TimeStringToInt(
            lineitems[5]
        )  # In the future. Could overflow 'int' eventually and fail somewhere.
    ...
    def ExpectedPosition(self, timeint=None, timedec=None):
        ...
        if timeint >= self.EndTime:
            timeint = (
                self.EndTime
            )  # Cannot proceed past the end of the planned trajectory.
            timedec = 0.0
        elif timeint < self.StartTime:
            timeint = (
                self.StartTime
            )  # Cannot extrapolate backwards in time, just hover at the start point.
            timedec = 0.0
        elapsedseconds = timeint - self.StartTime
...
class trajectory:
    ...
    def __init__(self, name):
        ...
        self.MotorName = (
            name  # The parent MotorName to match log messages with the parent motor.
        )
    ...
    def Clean(self):  # Trim expired entries from the trajectory list.
        ...
        while (
            len(self.TrajectoryList) > 0
            and self.TrajectoryList[0].EndTime < Clock.Now()
        ):
            LogFile.Log(
                "trajectory.Clean: Expired (",
                self.MotorName,
                self.TrajectoryList[0].Printable(),
                ")",
            )
            # temp = self.TrajectoryList.pop(0) # Remove the first entry from the list, it's not needed anymore.
            _ = self.TrajectoryList.pop(
                0
            )  # Remove the first entry from the list, it's not needed anymore.
        self.Validate()  # Is the trajectory useable?

    def Add(
        self, line
    ):  # Add new entry to the end of the list, any existing entries older than the new entry are trimmed.
        ...
        try:
            ...
            while (
                len(self.TrajectoryList) > 0
                and starttime <= self.TrajectoryList[-1].StartTime
            ):
                self.TrajectoryList = self.TrajectoryList[
                    :-1
                ]  # Remove last list entry, it's being replaced by new values.
            self.TrajectoryList.append(
                trajectorypoint(line)
            ...
            LogFile.Log(
                ...
                "): "
                + str(line)
                + ": Failed to create new trajectory point: "
                + str(e),
            )
            ExceptionCounter.Raise()  # Increment exception count for the session.
        self.Validate()  # Is the trajectory useable?
...
class steppermotor:
    ...
    def __init__(self, name):
        ...
        self.FaultSensitive = (
            False  # Set to TRUE to monitor the 'fault' pin on the DRV8825 chip.
        )
        ...
        self.StatusTimer = timer(
            name, 10
        )  # Set up an internal timer for sending status messages every 10 seconds. Can we overridden by RPi.
        ...
        self.WaitTime = (
            self.SlowTime
        )  # The time between pulses, starts slow, reduces as a move progresses. Resets every time a new target is set.
        ...
        self.SlewStepMultiplier = (
            1  # The number of steps/microsteps taken when making large SLEW moves.
        )
        ...
        self.SlewMode0 = (
            False  # Move0 pin setting when taking full steps in a large slew movement.
        )
        ...
        self.StepBCM = (
            None  # Set pin to OUTPUT. This sends the MOVE pulse to the controller.
        )
        self.DirectionBCM = (
            None  # Set pin to OUTPUT. This sets the MOVE direction to the controller.
        )
    ...
    def CheckOnTarget(self):
        ...
        if self.RequestedPosition == None:
            self.OnTarget = False
        elif self.RequestedPosition == self.CurrentPosition:
            self.OnTarget = True
        else:
            self.OnTarget = False
    ...
    def Reset(self, enable=False):
        ...
        if enable:
            self.EnableMotor()  # Leave motor enabled.
        else:
            self.DisableMotor()  # Disable the motor.
        self.OnTarget = False
        self.CurrentPosition = (
            self.RestPosition
        )  # Initialise CurrentPosition, we'll set it in next line. This is updated DURING moves too.
    ...
    def AddTrajectoryPoint(self, entry):
        try:
            ...
            ExceptionCounter.Raise()  # Increment exception count for the session.
        self.SendMotorStatus(
            immediate=True, codes="atp"
        )  # This triggers the next trajectory point faster than waiting for the regular status message will.
    ...
    def EnableMotor(self):
        ...
        if self.MotorConfigured:
            ...
            LogFile.Log(
                "steppermotor.EnableMotor: Motor is not configured. Will not enable."
            )
    ...
    def GoToAngle(self, newangle):
        if self.MotorConfigured:
            print("GoToAngle: Motor configured")
            LogFile.Log(
                "steppermotor.GoToAngle(" + self.MotorName + ") " + str(newangle)
            )
            print("GoToAngle: Stop")
            self.Stop()  # Clear any pre-existing trajectory before moving.
            print("GoToAngle: SetTargetByAngle")
            ...
            print("GoToAngle: Motor NOT configured")
            LogFile.Log(
                "steppermotor.GoToAngle("
                + self.MotorName
                + ") Rejected. Motor is not configured."
            )
            RPi.Write(
                "goto rejected "
                + self.MotorName
        ...
        print("GoToAngle: SendMotorStatus (immediate) gte")
        self.SendMotorStatus(
            immediate=True, codes="gte"
        )  # Tell RPi latest condition of the motor.
    ...
    def SetTargetByPosition(self, newposition=0, Limit=True):
        ...
        if Limit:  # OK to apply movement limits.
            ...
            if newangle < self.MinAngle:
                ...
                result = False  # Set failed.
        self.TargetPosition = (
            newposition  # Convert it into the nearest absolute STEP position.
        )
        self.WaitTime = (
            self.SlowTime
        )  # Start with slow move pulses. This reduces each time we call StepMove().
    ...
    def TargetFromTrajectoryPosition(self):
        ...
        if (
            targetposition != None and self.Trajectory.Valid and self.MotorConfigured
        ):  # We can set the target based upon current trajectory.
            result = self.SetTargetByPosition(targetposition)
        else:  # Target is just the current position. Config and Trajectory are invalid.
            LogFile.Log(
                "TargetFromTrajectoryPosition",
                self.MotorName,
                "not ready. tv,mc,ta=",
                self.Trajectory.Valid,
    ...
    def TunePosition(self, delta):
        ...
        if self.MotorConfigured:
            ...
            old = (
                self.CurrentPosition
            )  # Store the current position of the motor. We'll restore this when finished.
            new = (
                self.CurrentPosition + delta
            )  # Calculate the new target position (fullsteps).
            self.SetTargetByPosition(
                new, Limit=False
            )  # Set the target in the object. Primes it for the move, No error check on limits.
            LogFile.Log(
                "TunePosition("
                + self.MotorName
                + ") Current:"
            ...
            RPi.Write(
                ...
                + self.MotorName
                + " "
                + IntToTimeString(self.LatestTuneTime)
                + " "
                + str(delta)
                + " "
                + IntToTimeString(tunestarttime)
            )
            self.SendMotorStatus(
                immediate=True, codes="tup"
            )  # Tell RPi latest condition of the motor.
        else:
            RPi.Write(
                "tune rejected "
            ...
            LogFile.Log(
                "error : TunePosition("
                + self.MotorName
                + ") Rejected, motor is not yet configured."
            )

    def SetPins(
        self, stepBCM, directionBCM, mode0BCM, mode1BCM, mode2BCM, enableBCM, faultBCM
    ):
    ...
    def SetConfig(
        ...
        self.MinPosition = self.AngleToStep(
            self.MinAngle
        )  # Min clockwise movement. Location of limit switch in steps (This is self calibrating when in use).
        self.MaxPosition = self.AngleToStep(
            self.MaxAngle
        )  # Max clockwise movement. Location of limit switch in steps (This is self calibrating when in use).
    ...
    def ConfigureMotor(self, line):
        ...
        try:
            ...
            lc = len(lineitems)
            Clock.UpdateClockFromString(
                lineitems[2]
            )  # Check that the clock is as synchronised as possible.
            if lineitems[7] != "none":
                self.BacklashAngle = float(
                    lineitems[7]
                )  # Set new backlash angle for motor.
            if lineitems[8] != "none":
                self.Orientation = int(lineitems[8])  # Set new orientation for motor.
            if lineitems[9] != "none":
                self.FastTime = float(
                    lineitems[9]
                )  # Set new FASTEST PULSE time for motor.
            if lineitems[10] != "none":
                self.SlowTime = float(
                    lineitems[10]
                )  # Set new SLOWEST PULSE time for motor.
            if lineitems[11] != "none":
                self.TimeDelta = float(
                    lineitems[11]
                )  # Set new acceleration rate for motor.
            if self.WaitTime < self.FastTime:
                self.WaitTime = (
                    self.FastTime
                )  # Current motor speed cannot be faster than new fastest limit.
            if self.WaitTime > self.SlowTime:
                self.WaitTime = (
                    self.SlowTime
                )  # Current motor speed cannot be slower than new slowest limit.
            if lineitems[12] != "none":  # Must be within reasonable limits.
                ...
                self.StatusTimer = timer(
                    self.MotorName, temp
                )  # Set new repeat time for sending motor status messages back to the RPi
            if (
                lineitems[13] != "none"
            ):  # Enable/disable fault sensitivity. DRV8825 can then abort an observation.
            ...
            self.SlewStepMultiplier = int(
                lineitems[18]
            )  # Number of steps taken with a larget SLEW move (if microstepping in place).
            ...
            if lc > 21:  # Allow SLEW fast moves.
                self.SlewMotor = StringToBool(
                    lineitems[21]
                )  # Are SLEW fast moves allowed?
            if (
                lc > 22
            ):  # Load mode signals to select FULL STEPS when performing large fast slews.
    ...
    def StepMove(self, stepsize=1):
        ...
        if (
            self.MotorEnabled
        ):  # If we've disabled the motor, then perform everything except the move pulse.
        ...
        if self.OptimiseMoves:
            self.CurrentPosition = (
                self.CurrentPosition % self.AxisStepsPerRev
            )  # Can wrap around.
    ...
    def EfficiencyCheck(self, motorsteps):
        ...
        if abs(motorsteps) > int(
            self.AxisStepsPerRev / 2
        ):  # We're going the long way round.
            inversemove = self.InvertSteps(motorsteps)
            LogFile.Log(
                "steppermotor.EfficiencyCheck(): Inefficient move:",
                self.CurrentPosition,
                "to",
                self.TargetPosition,
                ",",
    ...
    def MoveMotorFast(self):
        ...
        if self.FaultBCM.GetValue() == False:  # DRV8825 'fault' pin is triggered.
            if self.FaultSensitive:  # The fault matters.
                if not self.FaultDetected:
                    ...
                    LogFile.Log(
                        "steppermotor.MoveMotorFast(",
                        self.MotorName,
                        ") DRV8825 fault - terminating.",
                    )
                return
        ...
        if abs(self.StepDir) != 1:  # self.StepDir must be +1 or -1
            LogFile.Log(
                "MoveMotorFast: "
                + self.MotorName
                + ' StepDir " + str(self.StepDir) + " is invalid. Must be +/-1'
            )
            return
        ...
        if self.StepDir != self.LastStepDir:  # We have a change of direction.
            LogFile.Log(
                ...
                + str(self.StepDir)
                + " vs "
                + str(self.LastStepDir)
                + "). Backlash?"
            )
        self.LastStepDir = (
            self.StepDir
        )  # Record the direction that the motor is moving in. This may be useful for handling gear backlash etc.
        if (
            self.SlewMotor
        ):  # We're allowed to make FAST moves using FULL STEPS if there's a long way to go.
            targettolerance = (
                100 * self.SlewStepMultiplier
            )  # This is as close as we want to get with large slew moves. Finetuning is done with MoveMotor() afterwards.
            ...
            while MotorSteps != 0:
                if (
                    self.CurrentPosition % self.SlewStepMultiplier == 0
                ):  # On FULLSTEP boundary. Switch to FULL STEPS.
                    break
                MotorSteps = (
                    MotorSteps - self.StepDir
                )  # REDUCE (-ve) the number of steps to take.
                self.StepMove(
                    stepsize=1
                )  # This will update CurrentPosition on-the-fly as the motor moves.
                self.SendMotorStatus(
                    codes="mov"
                )  # Long slow moves would cause RPi to trigger a reset and the user won't see progress until the end, so send regular status updates.
                for i in range(
                    1
                ):  # Check UART buffers. Can loop multiple times if you want to, but it pauses movement while checking.
            ...
            while MotorSteps != 0:
                if (
                    abs(self.TargetPosition - self.CurrentPosition) <= targettolerance
                ):  # We've got close with FULLSTEPS, switch back to microsteps.
                    break
                MotorSteps = MotorSteps - (
                    self.StepDir * self.SlewStepMultiplier
                )  # REDUCE (-ve) the number of steps to take.
                self.StepMove(
                    stepsize=self.SlewStepMultiplier
                )  # This will update CurrentPosition on-the-fly as the motor moves.
                self.SendMotorStatus(
                    codes="mov"
                )  # Long slow moves would cause RPi to trigger a reset and the user won't see progress until the end, so send regular status updates.
        ...
        ):  # Did the motor slew close to the intended position? (May not be the requested target if movement limits reached)
            LogFile.Log(
                "MoveMotorFast("
                + self.MotorName
                + "): End. CurrentPosition ("
                + str(self.CurrentPosition)
                + ") is NOT TargetPosition ("
                + str(self.TargetPosition)
                + ")!"
            )
        StatusLed.Task("idle")
    ...
    def MoveMotor(self):
        ...
        if (
            self.CurrentPosition != self.TargetPosition
        ):  # Did the motor reach intended position? (May not be the requested target if movement limits reached)
            LogFile.Log(
                "MoveMotor("
                + self.MotorName
                + "): End. CurrentPosition ("
    ...
    def ReportMotorConfig(self):
        ...
        line += (
            "usMode "
            + BoolToString(self.MicrosteppingMode0)
            + BoolToString(self.MicrosteppingMode1)
            + BoolToString(self.MicrosteppingMode2)
            + " "
        )  # Microstepping mode pin settings.
    ...
    def SendMotorStatus(self, immediate=False, codes="?-?"):
        ...
        """
        if (
            immediate or self.StatusTimer.Due()
        ):  # Only send the status at regular intervals, otherwise we flood communications.
            if (
                self.SendStatus == False
            ):  # Status message is currently disabled. Inform that we're not sending it.
                RPi.Write(
                    "# SendMotorStatus "
                    + IntToTimeString(Clock.Now())
                    + " "
            ...
            line += (
                IntToTimeString(self.Trajectory.ValidUntil()) + " "
            )  # When does the trajectory run out?
            line += (
                str(len(self.Trajectory.TrajectoryList)) + " "
            )  # How many segments in the trajectory?
            line += (
                str(self.CurrentPosition) + " "
            )  # Where is the camera at the moment?
            line += (
                str(self.CurrentDegrees()) + " "
            )  # Where is the camera at the moment?
            line += BoolToString(self.MotorConfigured) + " "  # MotorConfigured
            line += BoolToString(self.OnTarget) + " "  # Motor is on target or not.
            line += (
                str(self.WaitTime * 2) + " "
            )  # The pulse period (indicates speed) of the motor.
...
AzimuthFaultBCM.pull = (
    digitalio.Pull.UP
)  # Floating pins will toggle between FAULT and OK which causes chaos. Pull UP defaults to OK.
...
Azimuth.SetPins(
    stepBCM=AzimuthStepBCM,
    directionBCM=CommonDirectionBCM,
    mode0BCM=CommonMode0BCM,
    mode1BCM=CommonMode1BCM,
    mode2BCM=CommonMode2BCM,
    enableBCM=CommonEnableBCM,
    faultBCM=AzimuthFaultBCM,
)  # Direct control over Azimuth motor.
Azimuth.SetConfig(
    gearratio=(60 * 4),
...
Altitude.SetPins(
    stepBCM=AltitudeStepBCM,
    directionBCM=CommonDirectionBCM,
    mode0BCM=CommonMode0BCM,
    mode1BCM=CommonMode1BCM,
    mode2BCM=CommonMode2BCM,
    enableBCM=CommonEnableBCM,
    faultBCM=AltitudeFaultBCM,
)  # Direct control over Altitude motor.
Altitude.SetConfig(
    gearratio=(60 * 4),
    motorstepsperrev=400,
    minangle=0.0,
...
class picosession:
    def __init__(self):
        ...
        self.TrajectorySafetyms = (
            2 * 60 * 1000
        )  # How many milliseconds can a valid trajectory remain in use before comms failure terminates it? == 2 minutes.
    ...
    def MovePermission(self):
        ...
        if not Clock.ClockSynchronised:
            result = False  # Clock must be synchronised.
        for i in Motors:  # for ALL motors.
    ...
    def SendMotorStatus(self, motorname, immediate=False, codes="?-?"):
        ...
        for i in Motors:
            if i.MotorName == motorname:
                i.SendMotorStatus(immediate=immediate, codes=codes)
    ...
    def TrajectorySafety(self):
        ...
        try:
            elapsed = (
                RPi.ticks_ms() - RPi.LastRxms
            )  # How many ms elapsed since last receipt?
        except:
            elapsed = 0
            LogFile.Log(
                "TrajectorySafety: elapsed calculation failed.",
                RPi.ticks_ms(),
                RPi.LastRxms,
            )
            ExceptionCounter.Raise()  # Increment exception count for the session.
        if (
            self.TrajectorySafetyms != None and elapsed > self.TrajectorySafetyms
        ...
        if failsafe == False:
            self.FailsafeLatch = False  # Reset the latch.
    ...
    def SendSessionStatus(self, codes="?-?"):
        ...
        """
        if CircuitPythonVersion.split(".")[0] in ["7", "8"]:
            pass  # Supported CircuitPython version.
        else:  # Unexpected CircuitPython version, report it back.
            line = "# Expecting CircuitPython 7 or 8, found " + str(
                CircuitPythonVersion
            )
            RPi.Write(line)  # Send over UART to RPi.
        line = "session status "
        i = (
            time.time() - RPi.StartTime
        )  # Alive seconds. Use CPU clock not synchronised clock.
        line += IntToTimeString(Clock.Now()) + " "  # Current local timestamp.
        line += (
            BoolToString(Clock.ClockSynchronised) + " "
        )  # Do the RPi and microcontroller clocks agree?
        line += (
            BoolToString(self.AutonomousControl) + " "
        )  # Can motors drive themselves? Fully configured and trajectory known.
        line += (
            BoolToString(self.RemoteControl) + " "
        )  # Can motors be commanded remotely? Fully configured.
        line += str(i) + " "  # Alive seconds. Use CPU clock, not synchronised clock.
        line += (
            str(self.TrajectorySafetyFlushes) + " "
        )  # How many times has the trajectory been flushed for safety when comms failed?
        line += str(codes) + " "  # Add optional extra codes.
        line += (
            str(ExceptionCounter.Count) + " "
        )  # Append count of exceptions raised during operation.
        RPi.Write(line)  # Send over UART to RPi.
        line = "comms status "
        line += IntToTimeString(Clock.Now()) + " "  # Current local timestamp.
        line += (
            str(RPi.PicoRxErrors) + " "
        )  # How many messages were rejected from RPi by Microcontroller.
        line += (
            str(RPi.CharactersRead) + " "
        )  # How many bytes received from RPi by Microcontroller.
        line += (
            str(RPi.CharactersWritten) + " "
        )  # How many bytes written by Microcontroller to RPi.
        line += (
            str(RPi.WriteDrops) + " "
        )  # How many messages were dropped due to buffer overflow?
        line += (
            str(RPi.ReceiveAge()) + " "
        )  # Report how old the last received message is...
        line += (
            str(RPi.ReadDrops) + " "
        )  # How many received messages have been dropped because input buffer was full?
        line += (
            str(len(RPi.WriteQueue)) + " "
        )  # How many messages in the send queue currently? Checking for backlog building up.
        line += str(codes) + " "  # Add optional extra codes.
        RPi.Write(line)  # Send over UART to RPi.
    ...
    def AutoMoveMotors(self):  # Trigger movement of the motors.
        ...
        if self.AutonomousControl:
            ...
            for i in Motors:
                result = (
                    i.TargetFromTrajectoryPosition()
                )  # Set target for the motor based upon trajectory if available.
                if result:  # Target was successfully set.
                    if i.TargetPosition != i.CurrentPosition:
                        i.MoveMotor()
                else:  # Target was not successfuly set.
                    LogFile.Log(
                        "AutoMoveMotors",
                        i.MotorName,
                        "failed: TargetFromTrajectory returned",
                        result,
                    )
                    overallresult = False
        return overallresult
...
def PinCommand(line):
    ...
    for i in range(max(repeats, 1)):  # Loop as many times as requested.
        if (
            command == "on" and pinobj.Pin.direction == digitalio.Direction.OUTPUT
        ):  # Switch on and it's an OUTPUT. command
            ...
            if duration != 0:  # For a limited time only.
                ...
                if repeats > 0:
                    time.sleep(
                        duration
                    )  # If we're going to repeat, pause before repeating too.
        elif (
            command == "off" and pinobj.Pin.direction == digitalio.Direction.OUTPUT
        ):  # Switch off and it's an INPUT.
            pinobj.SetValue(False)  # Turn off.
            if duration != 0:  # For a limited time only.
...
def ProcessInput(line):
    ...
    if lineitems[0] == "exit":
        ...
        lineitems[0] == "sendstatus"
    ):  # Turn off status messages for motors. Useful when downloading a batch of trajectories, so no conflicting requests exchanged.
        RPi.Write("# " + line)
        for i in Motors:
            i.SendStatus = StringToBool(lineitems[2])
    elif line.startswith(
        "set rgb"
    ):  # A direct command to set a specific RGB LED color. (Debug support)
        ...
        for i in Motors:
            if i.MotorName == lineitems[3]:
                ...
                i.SendMotorStatus(
                    immediate=True, codes="cfg"
                )  # Immediately respond with lates motor status.
        Session.MovePermission()  # Decide if we have valid trajectories and configuration in every motor. OK to move if we do!
    elif (
        line.startswith == "report motor"
    ):  # RPi has requested the motor configurations to be reported back.
        for i in Motors:
            i.ReportMotorConfig()
...
class memorymanager:
    ...
    def Poll(
        self,
    ):  # Check current memory and trigger memory garbage collection early if needed.
...
try:
    while True:  # Full interaction
        ...
        try:
            Session.SendMotorStatus(
                "azimuth", codes="tmr"
            )  # Send azimuth status message.
        except Exception as e:
